  ...
  case $module in
    ...
    *)
      ...
      $omega "$@" -model:exec \
        ...
        -target:module amplitude_ward_ufo_physical_$module \
        -$mode "$process" 2>/dev/null
      $omega "$@" -model:exec \
        -target:parameter_module parameters_sm_ufo \
        -target:module amplitude_ward_ufo_unphysical_$module \
...
cat <<EOF
module interface_ward_ufo_${mode}_${module}
  use omega_interface
  use amplitude_ward_ufo_${mode}_${module}
  implicit none
  private
  public :: load
contains
...
end module interface_ward_ufo_${mode}_${module}
...
for module in $modules; do
    for mode in physical unphysical; do
cat <<EOF
  use interface_ward_ufo_${mode}_${module}, load_${mode}_${module} => load
EOF
    done
done
