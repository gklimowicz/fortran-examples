...
C***BEGIN PROLOGUE  RUMACH
C***PURPOSE  Compute the unit roundoff of the machine.
C***CATEGORY  R1
C***TYPE      SINGLE PRECISION (RUMACH-S, DUMACH-D)
...
C     number u such that  1.0 + u .ne. 1.0.  This is computed by RUMACH
C     in a machine-independent manner.
C
C***REFERENCES  (NONE)
C***ROUTINES CALLED  RUMSUM
C***REVISION HISTORY  (YYYYMMDD)
C   19930216  DATE WRITTEN
C   19930818  Added SLATEC-format prologue.  (FNF)
C   20030707  Added RUMSUM to force normal storage of COMP.  (ACH)
C***END PROLOGUE  RUMACH
C
      REAL U, COMP
C***FIRST EXECUTABLE STATEMENT  RUMACH
      U = 1.0E0
 10   U = U*0.5E0
      ...
      RUMACH = U*2.0E0
      RETURN
C----------------------- End of Function RUMACH ------------------------
      END
      SUBROUTINE RUMSUM(A,B,C)
C     Routine to force normal storing of A + B, for RUMACH.
      REAL A, B, C
      C = A + B
      RETURN
      END
...
C
C***SEE ALSO  SLSODE
C***ROUTINES CALLED  SGBFA, SGEFA, SVNORM
C***COMMON BLOCKS    SLS001
C***REVISION HISTORY  (YYMMDD)
C   791129  DATE WRITTEN
C   890501  Modified prologue to SLATEC/LDOC format.  (FNF)
...
C***BEGIN PROLOGUE  SSRCOM
C***SUBSIDIARY
C***PURPOSE  Save/restore ODEPACK COMMON blocks.
C***TYPE      SINGLE PRECISION (SSRCOM-S, DSRCOM-D)
C***AUTHOR  Hindmarsh, Alan C., (LLNL)
C***DESCRIPTION
C
C  This routine saves or restores (depending on JOB) the contents of
C  the COMMON block SLS001, which is used internally
...
C   930809  Renamed to allow single/double precision versions. (ACH)
C   010412  Reduced Common block length by 209+12. (ACH)
C   031105  Restored 'own' variables to Common block /SLS001/, to
C           enable interrupt/restart feature. (ACH)
C   031112  Added SAVE statement for data-loaded constants.
C***END PROLOGUE  SSRCOM
C**End
      INTEGER ISAV, JOB
      INTEGER ILS
...
C
C***FIRST EXECUTABLE STATEMENT  SSRCOM
...
C----------------------- END OF FUNCTION SVNORM ------------------------
      END
*DECK SIPREP
      SUBROUTINE SIPREP (NEQ, Y, RWORK, IA, JA, IPFLAG, F, JAC)
      EXTERNAL F, JAC
...
C Subroutine SPREP.  It is called only if MITER is 1 or 2.
...
C no trouble, and IPFLAG is the value of the SPREP error flag IPPER
C if there was trouble in Subroutine SPREP.
C-----------------------------------------------------------------------
      IPFLAG = 0
C Call SPREP to do matrix preprocessing operations. --------------------
      CALL SPREP (NEQ, Y, RWORK(LYH), RWORK(LSAVF), RWORK(LEWT),
     1   RWORK(LACOR), IA, JA, RWORK(LWM), RWORK(LWM), IPFLAG, F, JAC)
      LENWK = MAX(LREQ,LWMIN)
      IF (IPFLAG .LT. 0) RETURN
C If SPREP was successful, move YH to end of required space for WM. ----
      LYHN = LWM + LENWK
...
C If ISTATE = 1, move EWT (left) to its new position. ------------------
 ...
 40   LEWT = LEWTN
      RETURN
C----------------------- End of Subroutine SIPREP ----------------------
      END
*DECK SPREP
      SUBROUTINE SPREP (NEQ, Y, YH, SAVF, EWT, FTEM, IA, JA,
...
C In addition to variables described previously, SPREP uses the
...
C IYS    = flag value from SODRV or SCDRV.
...
C         -3  insufficient storage for SODRV.
C         -4  other error flag from SODRV (should never occur).
C         -5  insufficient storage for SCDRV.
C         -6  other error flag from SCDRV.
C-----------------------------------------------------------------------
      IBIAN = LRAT*2
      IPIAN = IBIAN + 1
      NP1 = N + 1
...
C Compute new ordering of rows/columns of Jacobian. --------------------
 ...
 170    IWK(IBR+I) = I
      NSP = LIWK + 1 - IPISP
      CALL SODRV (N, IWK(IPIAN), IWK(IPJAN), WK, IWK(IPR), IWK(IPIC),
...
C Reorder JAN and do symbolic LU factorization of matrix. --------------
 ...
 180    WK(IBA+I) = 0.0E0
      ...
      CALL SCDRV (N,IWK(IPR),IWK(IPC),IWK(IPIC),IWK(IPIAN),IWK(IPJAN),
...
C sparse matrix reordering operation in SODRV.
C-----------------------------------------------------------------------
...
C----------------------- End of Subroutine CNTNZU ----------------------
      END
*DECK SPRJS
      SUBROUTINE SPRJS (NEQ,Y,YH,NYH,EWT,FTEM,SAVF,WK,IWK,F,JAC)
      EXTERNAL F,JAC
      INTEGER NEQ, NYH, IWK
     ...
     1   SRUR, SVNORM
C-----------------------------------------------------------------------
C SPRJS is called to compute and process the matrix
C P = I - H*EL(1)*J , where J is an approximation to the Jacobian.
...
C is 1 or 2, the P matrix is subjected to LU decomposition in SCDRV.
C P and its LU decomposition are stored (separately) in WK.
C
C In addition to variables described previously, communication
C with SPRJS uses the following:
C Y     = array containing predicted values on entry.
C FTEM  = work array of length N (ACOR in SSTODE).
C SAVF  = array containing f evaluated at predicted y.
...
C       = 0 if no error.
C       = 1  if zero pivot found in SCDRV.
C       = 2  if a singular matrix arose with MITER = 3.
C       = -1 if insufficient storage for SCDRV (should not occur here).
C       = -2 if other error found in SCDRV (should not occur here).
...
C If MITER = 3, construct a diagonal approximation to J and P. ---------
 ...
 330  IERPJ = 2
      RETURN
C----------------------- End of Subroutine SPRJS -----------------------
      END
*DECK SSOLSS
      SUBROUTINE SSOLSS (WK, IWK, X, TEM)
      INTEGER IWK
      REAL WK, X, TEM
...
C This routine manages the solution of the linear system arising from
C a chord iteration.  It is called if MITER .ne. 0.
C If MITER is 1 or 2, it calls SCDRV to accomplish this.
C If MITER = 3 it updates the coefficient H*EL0 in the diagonal
C matrix, and then computes the solution.
C communication with SSOLSS uses the following variables:
...
C         IERSL = -1 if SCDRV returned an error flag (MITER = 1 or 2).
...
C-----------------------------------------------------------------------
 ...
 100  CALL SCDRV (N,IWK(IPR),IWK(IPC),IWK(IPIC),IWK(IPIAN),IWK(IPJAN),
...
C
 ...
 390  IERSL = 1
      RETURN
C
C----------------------- End of Subroutine SSOLSS ----------------------
      END
*DECK SSRCMS
      SUBROUTINE SSRCMS (RSAV, ISAV, JOB)
C-----------------------------------------------------------------------
C This routine saves or restores (depending on JOB) the contents of
C the Common blocks SLS001, SLSS01, which are used
...
c                                                                 5/2/83
c***********************************************************************
c  sodrv -- driver for sparse matrix reordering routines
c***********************************************************************
c
c  description
c
c    sodrv finds a minimum degree ordering of the rows and columns
...
c
c    note.. sodrv and its subordinate routines have been modified to
...
c    modifications to the original sodrv module have been made in
c    the coding in subroutine mdi, and in the initial comments in
c    subroutines sodrv and md.
c
c    if only the nonzero entries in the upper triangle of m are being
c    stored, then sodrv symmetrically reorders (ia,ja,a), (optionally)
...
c             10n+1  insufficient storage in sodrv
c             11n+1  illegal path specification
c
c
c  conversion from real to double precision
c
c    change the real declarations in sodrv and ssro to double precision
c    declarations.
c
c-----------------------------------------------------------------------
c
...
c***********************************************************************
c  ssro -- symmetric reordering of sparse symmetric matrix
c***********************************************************************
c
c  description
c
...
c
c    ssro does not rearrange the order of the rows, but does move
...
c    i), then the driver will call a subroutine (snroc) which rearranges
...
c    to the new ordering.  if  path.ne.1,  then snroc is assumed to have
...
c    three stages --
c     snsfc -- the matrix m is processed symbolically to determine where
c               fillin will occur during the numeric factorization.
c     snnfc -- the matrix m is factored numerically into the product ldu
c               of a unit lower triangular matrix l, a diagonal matrix
c               d, and a unit upper triangular matrix u, and the system
c               mx = b  is solved.
c     snnsc -- the linear system  mx = b  is solved using the ldu
c  or           factorization from snnfc.
c     snntc -- the transposed linear system  mt x = b  is solved using
c               the ldu factorization from nnf.
c    for several systems whose coefficient matrices have the same
c    nonzero structure, snsfc need be done only once (for the first
c    system).  then snnfc is done once for each additional system.  for
c    several systems with the same coefficient matrix, snsfc and snnfc
c    need be done only once (for the first system).  then snnsc or snntc
c    is done once for each additional right-hand side.
c
c nv    - path  - path specification.  values and their meanings are --
c       -           1  perform snroc, snsfc, and snnfc.
c       -           2  perform snnfc only (snsfc is assumed to have been
c       -               done in a manner compatible with the storage
c       -               allocation used in the driver).
c       -           3  perform snnsc only (snsfc and snnfc are assumed
c       -               to have been done in a manner compatible with
c       -               the storage allocation used in the driver).
c       -           4  perform snntc only (snsfc and snnfc are assumed
c       -               to have been done in a manner compatible with
c       -               the storage allocation used in the driver).
c       -           5  perform snroc and snsfc.
c
...
c       -            3n+k   insufficient storage in snsfc  --  row = k
c       -            4n+1   insufficient storage in snnfc
c       -            5n+k   null pivot  --  row = k
c       -            6n+k   insufficient storage in snsfc  --  row = k
c       -            7n+1   insufficient storage in snnfc
c       -            8n+k   zero pivot  --  row = k
c       -           10n+1   insufficient storage in scdrv
c       -           11n+1   illegal path specification
c
...
c******initialize and divide up temporary storage  *******************
      ...
      jl   = jrl +   n
c
c  ******  reorder a if necessary, call snsfc if flag is set  **********
...
c
   ...
   8  return
c
c ** error.. error detected in snroc, snsfc, snnfc, or snnsc
 100  return
c ** error.. insufficient storage
 110  flag = 10*n + 1
      return
c ** error.. illegal path specification
 111  flag = 11*n + 1
      ...
      subroutine snroc (n, ic, ia, ja, a, jar, ar, p, flag)
c
c       ----------------------------------------------------------------
c
...
c    is used, then snroc should be used to reorder the coefficient
c    matrix.  the calling sequence is --
c        (       (matrix ordering))
c        (snroc  (matrix reordering))
c         snsfc  (symbolic factorization to determine where fillin will
c                  occur during numeric factorization)
c         snnfc  (numeric factorization into product ldu of unit lower
c                  triangular matrix l, diagonal matrix d, and unit
c                  upper triangular matrix u, and solution of linear
c                  system)
c         snnsc  (solution of linear system for additional right-hand
c                  side using ldu factorization from snnfc)
...
c         in snsfc, r and ic can be the same array in the calling
c    sequence if no reordering of the coefficient matrix has been done.
c         in snnfc, r, c, and ic can all be the same array if no
...
c         in snnsc or snntc, r and c can be the same array if no
...
c       -           the number of nonzeros in the strict lower triangle
c       -           of m plus fillin  (il(n+1)-1 after snsfc).
c nv    - n     - number of variables/equations.
c nva   - r     - ordering of the rows of m.
c       -           size = n.
...
c       -           the number of nonzeros in the strict upper triangle
c       -           of m plus fillin  (iu(n+1)-1 after snsfc).
c fra   - z     - solution x.
c       -           size = n.
c
c       ----------------------------------------------------------------
...
c
      ...
      real tmp(*), lki, sum, dk
c     double precision  a(*), l(*), d(*), u(*), z(*), b(*), row(*)
c     double precision  tmp(*), lki, sum, dk
c
...
*DECK SSTODA
     ...
     3   MAXORD, MAXCOR, MSBP, MXNCF, N, NQ, NST, NFE, NJE, NQU
      ...
      REAL CONIT, CRATE, EL, ELCO, HOLD, RMAX, TESCO,
     2   CCMAX, EL0, H, HMIN, HMXI, HU, RC, TN, UROUND
      REAL ROWND2, CM1, CM2, PDEST, PDLAST, RATIO,
     1   PDNORM
      COMMON /SLS001/ CONIT, CRATE, EL(13), ELCO(13,12),
     1   HOLD, RMAX, TESCO(3,12),
     2   CCMAX, EL0, H, HMIN, HMXI, HU, RC, TN, UROUND,
     ...
     6   MAXORD, MAXCOR, MSBP, MXNCF, N, NQ, NST, NFE, NJE, NQU
      COMMON /SLSA01/ ROWND2, CM1(12), CM2(5), PDEST, PDLAST, RATIO,
     1   PDNORM,
     2   IOWND2(3), ICOUNT, IRFLAG, JTYP, MUSED, MXORDN, MXORDS
      INTEGER I, I1, IREDO, IRET, J, JB, M, NCF, NEWQ
      INTEGER LM1, LM1P1, LM2, LM2P1, NQM1, NQM2
      REAL DCON, DDN, DEL, DELP, DSM, DUP, EXDN, EXSM, EXUP,
     1   R, RH, RHDN, RHSM, RHUP, TOLD, SMNORM
      REAL ALPHA, DM1,DM2, EXM1,EXM2,
     1   PDH, PNORM, RATE, RH1, RH1IT, RH2, RM, SM1(12)
      SAVE SM1
      DATA SM1/0.5E0, 0.575E0, 0.55E0, 0.45E0, 0.35E0, 0.25E0,
     1   0.20E0, 0.15E0, 0.10E0, 0.075E0, 0.050E0, 0.025E0/
C-----------------------------------------------------------------------
C SSTODA performs one step of the integration of an initial value
C problem for a system of ordinary differential equations.
C Note: SSTODA is independent of the value of the iteration method
C indicator MITER, when this is .ne. 0, and hence is independent
C of the type of chord method used, or the Jacobian structure.
C Communication with SSTODA is done with the following variables:
C
C Y      = an array of length .ge. N used as the Y argument in
C          all calls to F and JAC.
C NEQ    = integer array containing problem size in NEQ(1), and
...
C          to Jacobian type JT.  (The SLSODA/SLSODAR argument JT is
C          communicated here as JTYP, but is not used in SSTODA
C          except to load MITER following a method switch.)
C          MITER may be reset by SSTODA.
C N      = the number of first-order differential equations.
C-----------------------------------------------------------------------
      KFLAG = 0
      TOLD = TN
...
C for the next increase.
C SCFODE is called to get the needed coefficients for both methods.
C-----------------------------------------------------------------------
      LMAX = MAXORD + 1
      NQ = 1
      L = 2
...
*DECK SPRJA
     ...
     4   LYH, LEWT, LACOR, LSAVF, LWM, LIWM, METH, MITER,
     5   MAXORD, MAXCOR, MSBP, MXNCF, N, NQ, NST, NFE, NJE, NQU
      COMMON /SLSA01/ ROWND2, ROWNS2(20), PDNORM,
     1   IOWND2(3), IOWNS2(2), JTYP, MUSED, MXORDN, MXORDS
      INTEGER I, I1, I2, IER, II, J, J1, JJ, LENP,
     1   MBA, MBAND, MEB1, MEBAND, ML, ML3, MU, NP1
      REAL CON, FAC, HL0, R, R0, SRUR, YI, YJ, YJJ,
     1   SMNORM, SFNORM, SBNORM
C-----------------------------------------------------------------------
C SPRJA is called by SSTODA to compute and process the matrix
C P = I - H*EL(1)*J , where J is an approximation to the Jacobian.
C Here J is computed by the user-supplied routine JAC if
...
C by SMNORM) is computed, and J is overwritten by P.  P is then
C subjected to LU decomposition in preparation for later solution
C of linear systems with P as coefficient matrix.  This is done
C by SGEFA if MITER = 1 or 2, and by SGBFA if MITER = 4 or 5.
C
C In addition to variables described previously, communication
C with SPRJA uses the following:
C Y     = array containing predicted values on entry.
C FTEM  = work array of length N (ACOR in SSTODA).
C SAVF  = array containing f evaluated at predicted y.
C WM    = real work space for matrices.  On output it contains the
...
C of the vector of length N contained in the array V, with weights
C contained in the array w of length N:
C   SMNORM = MAX(i=1,...,N) ABS(V(i))*W(i)
C-----------------------------------------------------------------------
      INTEGER N,   I
      REAL V, W,   VM
      DIMENSION V(N), W(N)
...
C   SFNORM = MAX(i=1,...,N) ( W(i) * Sum(j=1,...,N) ABS(a(i,j))/W(j) )
C-----------------------------------------------------------------------
      INTEGER N,   I, J
      REAL A,   W, AN, SUM
      DIMENSION A(N,N), W(N)
...
C   SBNORM = MAX(i=1,...,N) ( W(i) * Sum(j=1,...,N) ABS(a(i,j))/W(j) )
C-----------------------------------------------------------------------
      INTEGER N, NRA, ML, MU
 ...
 20     CONTINUE
      SBNORM = AN
      RETURN
C----------------------- End of Function SBNORM ------------------------
      END
*DECK SSRCMA
      SUBROUTINE SSRCMA (RSAV, ISAV, JOB)
C-----------------------------------------------------------------------
C This routine saves or restores (depending on JOB) the contents of
C the Common blocks SLS001, SLSA01, which are used
C internally by one or more ODEPACK solvers.
C
...
C
      RETURN
C----------------------- End of Subroutine SSRCMA ----------------------
      END
*DECK SRCHEK
      SUBROUTINE SRCHEK (JOB, G, NEQ, Y, YH,NYH, G0, G1, GX, JROOT, IRT)
      EXTERNAL G
...
C Subroutine SROOTS to locate the root as precisely as possible.
C
C In addition to variables described previously, SRCHEK
C uses the following for communication:
C JOB    = integer flag indicating type of call:
C          JOB = 1 means the problem is being initialized, and SRCHEK
C                  is to look for a root at or very near the initial T.
C          JOB = 2 means a continuation call to the solver was just
C                  made, and SRCHEK is to check for a root in the
C                  relevant part of the step last taken.
C          JOB = 3 means a successful step was just taken, and SRCHEK
...
C          T0 is updated by SRCHEK, whether a root is found or not.
...
C-----------------------------------------------------------------------
      ...
      DO 10 I = 1,NGC
 10     JROOT(I) = 0
      HMING = (ABS(TN) + ABS(H))*UROUND*100.0E0
C
...
C g has a zero at T.  Look at g at T + (small increment). --------------
 ...
 120    Y(I) = Y(I) + TEMP2*YH(I,2)
      ...
      ZROOT = .FALSE.
      DO 130 I = 1,NGC
 130    IF (ABS(G0(I)) .LE. 0.0E0) ZROOT = .TRUE.
      IF (.NOT. ZROOT) GO TO 190
...
C If a root was found on the previous step, evaluate G0 = g(T0). -------
      CALL SINTDY (T0, 0, YH, NYH, Y, IFLAG)
      CALL G (NEQ, T0, Y, NGC, G0)
      NGE = NGE + 1
      ZROOT = .FALSE.
      DO 210 I = 1,NGC
 210    IF (ABS(G0(I)) .LE. 0.0E0) ZROOT = .TRUE.
      IF (.NOT. ZROOT) GO TO 260
C g has a zero at T0.  Look at g at T + (small increment). -------------
      TEMP1 = SIGN(HMING,H)
      T0 = T0 + TEMP1
      IF ((T0 - TN)*H .LT. 0.0E0) GO TO 230
      TEMP2 = TEMP1/H
      DO 220 I = 1,N
 220    Y(I) = Y(I) + TEMP2*YH(I,2)
      GO TO 240
 230  CALL SINTDY (T0, 0, YH, NYH, Y, IFLAG)
 240  CALL G (NEQ, T0, Y, NGC, G0)
      NGE = NGE + 1
      ZROOT = .FALSE.
      DO 250 I = 1,NGC
        IF (ABS(G0(I)) .GT. 0.0E0) GO TO 250
        JROOT(I) = 1
        ZROOT = .TRUE.
 250    CONTINUE
      IF (.NOT. ZROOT) GO TO 260
...
C Set T1 to TN or TOUTC, whichever comes first, and get g at T1. -------
      ...
      IF ((TOUTC - TN)*H .GE. 0.0E0) GO TO 310
      T1 = TOUTC
      IF ((T1 - T0)*H .LE. 0.0E0) GO TO 390
      CALL SINTDY (T1, 0, YH, NYH, Y, IFLAG)
      GO TO 330
 310  T1 = TN
      DO 320 I = 1,N
 320    Y(I) = YH(I,1)
 330  CALL G (NEQ, T1, Y, NGC, G1)
      NGE = NGE + 1
C Call SROOTS to search for root in interval from T0 to T1. ------------
      JFLAG = 0
 350  CONTINUE
      CALL SROOTS (NGC, HMING, JFLAG, T0, T1, G0, G1, GX, X, JROOT)
      IF (JFLAG .GT. 1) GO TO 360
      CALL SINTDY (X, 0, YH, NYH, Y, IFLAG)
      CALL G (NEQ, X, Y, NGC, GX)
      NGE = NGE + 1
      GO TO 350
 360  T0 = X
      CALL SCOPY (NGC, GX, 1, G0, 1)
...
C
 390  CONTINUE
      RETURN
C----------------------- End of Subroutine SRCHEK ----------------------
      END
*DECK SROOTS
      SUBROUTINE SROOTS (NG, HMIN, JFLAG, X0, X1, G0, G1, GX, X, JROOT)
      INTEGER NG, JFLAG, JROOT
      REAL HMIN, X0, X1, G0, G1, GX, X
      DIMENSION G0(NG), G1(NG), GX(NG), JROOT(NG)
      INTEGER IOWND3, IMAX, LAST, IDUM3
      REAL ALPHA, X2, RDUM3
      COMMON /SLSR01/ ALPHA, X2, RDUM3(3),
...
C are communicated through the call sequence of SROOTS.
...
C          JFLAG = 1 means SROOTS needs a value of g(x).  Set GX = g(X)
C                    and call SROOTS again.
...
C-----------------------------------------------------------------------
      ...
      REAL T2, TMAX, FRACINT, FRACSUB, ZERO,HALF,TENTH,FIVE
      LOGICAL ZROOT, SGNCHG, XROOT
      SAVE ZERO, HALF, TENTH, FIVE
      DATA ZERO/0.0E0/, HALF/0.5E0/, TENTH/0.1E0/, FIVE/5.0E0/
C
...
C At this point, G0(i) has been checked and cannot be zero. ------------
 110    IF (SIGN(1.0E0,G0(I)) .EQ. SIGN(1.0E0,G1(I))) GO TO 120
...
C Neither G0(i) nor GX(i) can be zero at this point. -------------------
 210    IF (SIGN(1.0E0,G0(I)) .EQ. SIGN(1.0E0,GX(I))) GO TO 220
...
C Return with X1 as the root.  Set JROOT.  Set X = X1 and GX = G1. -----
 300  JFLAG = 2
      X = X1
      CALL SCOPY (NG, G1, 1, GX, 1)
      DO 320 I = 1,NG
        JROOT(I) = 0
        IF (ABS(G1(I)) .GT. ZERO) GO TO 310
          JROOT(I) = 1
          GO TO 320
 310    IF (SIGN(1.0E0,G0(I)) .NE. SIGN(1.0E0,G1(I))) JROOT(I) = 1
 320    CONTINUE
      RETURN
C
...
C No sign changes in this interval.  Set X = X1, return JFLAG = 4. -----
 420  CALL SCOPY (NG, G1, 1, GX, 1)
      X = X1
      JFLAG = 4
      RETURN
C----------------------- End of Subroutine SROOTS ----------------------
      END
*DECK SSRCAR
      SUBROUTINE SSRCAR (RSAV, ISAV, JOB)
C-----------------------------------------------------------------------
C This routine saves or restores (depending on JOB) the contents of
C the Common blocks SLS001, SLSA01, SLSR01, which are used
C internally by one or more ODEPACK solvers.
C
...
C
      RETURN
C----------------------- End of Subroutine SSRCAR ----------------------
      END
*DECK SSTODPK
      SUBROUTINE SSTODPK (NEQ, Y, YH, NYH, YH1, EWT, SAVF, SAVX, ACOR,
     1   WM, IWM, F, JAC, PSOL)
      EXTERNAL F, JAC, PSOL
      INTEGER NEQ, NYH, IWM
      REAL Y, YH, YH1, EWT, SAVF, SAVX, ACOR, WM
      DIMENSION NEQ(*), Y(*), YH(NYH,*), YH1(*), EWT(*), SAVF(*),
     1   SAVX(*), ACOR(*), WM(*), IWM(*)
      INTEGER IOWND, IALTH, IPUP, LMAX, MEO, NQNYH, NSLP,
     1   ICF, IERPJ, IERSL, JCUR, JSTART, KFLAG, L,
...
C-----------------------------------------------------------------------
C SSTODPK performs one step of the integration of an initial value
C problem for a system of Ordinary Differential Equations.
C-----------------------------------------------------------------------
C The following changes were made to generate Subroutine SSTODPK
C from Subroutine SSTODE:
C 1. The array SAVX was added to the call sequence.
C 2. PJAC and SLVS were replaced by PSOL in the call sequence.
C 3. The Common block /SLPK01/ was added for communication.
...
C 6. The call to PJAC was replaced with a call to SPKSET (fixed name),
...
C 8. SLVS was renamed SSOLPK and includes NEQ, SAVX, EWT, F, and JAC.
C    SAVX was added because SSOLPK now needs Y and SAVF undisturbed.
C 9. The nonlinear convergence failure count NCFN is set at 430.
C-----------------------------------------------------------------------
C Note: SSTODPK is independent of the value of the iteration method
C indicator MITER, when this is .ne. 0, and hence is independent
C of the type of chord method used, or the Jacobian structure.
C Communication with SSTODPK is done with the following variables:
C
C NEQ    = integer array containing problem size in NEQ(1), and
C          passed as the NEQ argument in all calls to F and JAC.
C Y      = an array of length .ge. N used as the Y argument in
...
C WM,IWM = real and integer work arrays associated with matrix
C          operations in chord iteration (MITER .ne. 0).
C CCMAX  = maximum relative change in H*EL0 before SPKSET is called.
C H      = the step size to be attempted on the next step.
C          H is altered by the error control algorithm during the
C          problem.  H can be either positive or negative, but its
C          sign must remain constant throughout the problem.
...
C              -1  the requested error could not be achieved.
C              -2  corrector convergence could not be achieved.
C              -3  fatal error in SPKSET or SSOLPK.
C          A return with KFLAG = -1 or -2 means either
C          ABS(H) = HMIN or 10 consecutive failures occurred.
C          On a return with KFLAG negative, the values of TN and
C          the YH array are as of the beginning of the last
...
C MAXORD = the maximum order of integration method to be allowed.
C MAXCOR = the maximum number of corrector iterations allowed.
C MSBP   = maximum number of steps between SPKSET calls (MITER .gt. 0).
C MXNCF  = maximum number of convergence failures allowed.
C METH/MITER = the method flags.  See description in driver.
C N      = the number of first-order differential equations.
C-----------------------------------------------------------------------
...
C (JACFLG .ne. 0) or not (JACFLG = 0), to trigger a call to SPKSET.
...
C-----------------------------------------------------------------------
 200  IF (JACFLG .NE. 0) GO TO 202
      ...
      CRATE = 0.7E0
      GO TO 205
 202  IF (ABS(RC-1.0E0) .GT. CCMAX) IPUP = MITER
      IF (NST .GE. NSLP+MSBP) IPUP = MITER
 205  TN = TN + H
      I1 = NQNYH + 1
      DO 215 JB = 1,NQ
...
C-----------------------------------------------------------------------
 ...
 230    Y(I) = YH(I,1)
      ...
      IF (IPUP .LE. 0) GO TO 250
C-----------------------------------------------------------------------
C If indicated, SPKSET is called to update any matrix data needed,
C before starting the corrector iteration.
C IPUP is set to 0 as an indicator that this has been done.
C-----------------------------------------------------------------------
      CALL SPKSET (NEQ, Y, YH1, EWT, ACOR, SAVF, WM, IWM, F, JAC)
      IPUP = 0
      RC = 1.0E0
      NSLP = NST
      CRATE = 0.7E0
...
C-----------------------------------------------------------------------
 ...
 360    SAVX(I) = H*SAVF(I) - (YH(I,2) + ACOR(I))
      CALL SSOLPK (NEQ, Y, SAVF, SAVX, EWT, WM, IWM, F, PSOL)
      IF (IERSL .LT. 0) GO TO 430
      IF (IERSL .GT. 0) GO TO 410
      DEL = SVNORM (N, SAVX, EWT)
      DO 380 I = 1,N
...
C-----------------------------------------------------------------------
 400  IF (M .NE. 0) CRATE = MAX(0.2E0*CRATE,DEL/DELP)
      DCON = DEL*MIN(1.0E0,1.5E0*CRATE)/EPCON
      IF (DCON .LE. 1.0E0) GO TO 450
      M = M + 1
      IF (M .EQ. MAXCOR) GO TO 410
      IF (M .GE. 2 .AND. DEL .GT. 2.0E0*DELP) GO TO 410
...
C-----------------------------------------------------------------------
C The corrector iteration failed to converge.
C If MITER .ne. 0 and the Jacobian is out of date, SPKSET is called for
C the next try.  Otherwise the YH array is retracted to its values
C before prediction, and H is reduced, if possible.  If H cannot be
C reduced or MXNCF failures have occurred, exit with KFLAG = -2.
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
 ...
 720  HOLD = H
      JSTART = 1
      RETURN
C----------------------- End of Subroutine SSTODPK ---------------------
      END
*DECK SPKSET
      SUBROUTINE SPKSET (NEQ, Y, YSV, EWT, FTEM, SAVF, WM, IWM, F, JAC)
      EXTERNAL F, JAC
      INTEGER NEQ, IWM
      REAL Y, YSV, EWT, FTEM, SAVF, WM
     ...
     2   NNI, NLI, NPS, NCFN, NCFL
C-----------------------------------------------------------------------
C SPKSET is called by SSTODPK to interface with the user-supplied
...
C
C In addition to variables described previously, communication
C with SPKSET uses the following:
C Y     = array containing predicted values on entry.
C YSV   = array containing predicted y, to be saved (YH1 in SSTODPK).
C FTEM  = work array of length N (ACOR in SSTODPK).
C SAVF  = array containing f evaluated at predicted y.
...
C
     ...
     1   WM(LOCWP), IWM(LOCIWP), IER)
      ...
      IERPJ = 1
      RETURN
C----------------------- End of Subroutine SPKSET ----------------------
      END
*DECK SSOLPK
      SUBROUTINE SSOLPK (NEQ, Y, SAVF, X, EWT, WM, IWM, F, PSOL)
      EXTERNAL F, PSOL
      INTEGER NEQ, IWM
      REAL Y, SAVF, X, EWT, WM
     ...
     2   NNI, NLI, NPS, NCFN, NCFL
C-----------------------------------------------------------------------
C This routine interfaces to one of SSPIOM, SSPIGMR, SPCG, SPCGS, or
C SUSOL, for the solution of the linear system arising from a Newton
C iteration.  It is called if MITER .ne. 0.
C In addition to variables described elsewhere,
C communication with SSOLPK uses the following variables:
...
C-----------------------------------------------------------------------
 100  CONTINUE
      ...
      CALL SSPIOM (NEQ, TN, Y, SAVF, WM(LB), EWT, N, MAXL, KMP, DELTA,
...
C-----------------------------------------------------------------------
 200  CONTINUE
      ...
      CALL SCOPY (N, X, 1, WM(LB), 1)
      CALL SSCAL (N, RSQRTN, EWT, 1)
      CALL SSPIGMR (NEQ, TN, Y, SAVF, WM(LB), EWT, N, MAXL, MAXLP1, KMP,
     ...
     2   WM(LQ), LGMR, WM(LOCWP), IWM(LOCIWP), WM(LWK), WM(LDL), IFLAG)
      ...
      RETURN
C-----------------------------------------------------------------------
C Use SPCG to solve the linear system P*x = -f
C-----------------------------------------------------------------------
 300  CONTINUE
      LR = 1
      LP = LR + N
      ...
      LZ = LW + N
      LWK = LZ + N
      CALL SCOPY (N, X, 1, WM(LR), 1)
      CALL SPCG (NEQ, TN, Y, SAVF, WM(LR), EWT, N, MAXL, DELTA, HL0,
     ...
     2          LPCG, WM(LOCWP), IWM(LOCIWP), WM(LWK), IFLAG)
      ...
      RETURN
C-----------------------------------------------------------------------
C Use SPCGS to solve the linear system P*x = -f
C-----------------------------------------------------------------------
 400  CONTINUE
      LR = 1
      LP = LR + N
      ...
      LWK = LZ + N
      CALL SCOPY (N, X, 1, WM(LR), 1)
      CALL SPCGS (NEQ, TN, Y, SAVF, WM(LR), EWT, N, MAXL, DELTA, HL0,
     1           JPRE, MNEWT, F, PSOL, NPSL, X, WM(LP), WM(LW), WM(LZ),
     2           LPCG, WM(LOCWP), IWM(LOCIWP), WM(LWK), IFLAG)
      NNI = NNI + 1
      NLI = NLI + LPCG
      ...
      RETURN
C-----------------------------------------------------------------------
C Use SUSOL, which interfaces to PSOL, to solve the linear system
...
C-----------------------------------------------------------------------
 900  CONTINUE
      ...
      LWK = LB + N
      CALL SCOPY (N, X, 1, WM(LB), 1)
      CALL SUSOL (NEQ, TN, Y, SAVF, WM(LB), EWT, N, DELTA, HL0, MNEWT,
     1   PSOL, NPSL, X, WM(LOCWP), IWM(LOCIWP), WM(LWK), IFLAG)
      ...
      IF (IFLAG .LT. 0) IERSL = -1
      RETURN
C----------------------- End of Subroutine SSOLPK ----------------------
      END
*DECK SSPIOM
      SUBROUTINE SSPIOM (NEQ, TN, Y, SAVF, B, WGHT, N, MAXL, KMP, DELTA,
     ...
     2            LIOM, WP, IWP, WK, IFLAG)
      ...
      REAL TN,Y,SAVF,B,WGHT,DELTA,HL0,X,V,HES,WP,WK
...
C                (B is the same as V(*,MAXL+1) in the call to SSPIOM.)
C
...
C
C           WK = real work array of length N used by SATV and PSOL.
C
...
C                It is loaded in SHEFA and used in SHESL.
C
...
C-----------------------------------------------------------------------
      ...
      REAL BNRM, BNRM0, PROD, RHO, SNORMW, SNRM2, TEM
C
      IFLAG = 0
      LIOM = 0
      NPSL = 0
...
C-----------------------------------------------------------------------
      ...
      DO 90 LL = 1,MAXL
        LIOM = LL
C-----------------------------------------------------------------------
C Call routine SATV to compute VNEW = Abar*v(l), where Abar is
C the matrix A with scaling and inverse preconditioner factors applied.
C Call routine SORTHOG to orthogonalize the new vector vnew = V(*,l+1).
C Call routine SHEFA to update the factors of HES.
C-----------------------------------------------------------------------
        CALL SATV (NEQ, Y, SAVF, V(1,LL), WGHT, X, F, PSOL, V(1,LL+1),
     1        WK, WP, IWP, HL0, JPRE, IER, NPSL)
        IF (IER .NE. 0) GO TO 300
        CALL SORTHOG (V(1,LL+1), V, HES, N, LL, MAXL, KMP, SNORMW)
        CALL SHEFA (HES, MAXL, LL, IPVT, INFO, LL)
        LM1 = LL - 1
...
C-----------------------------------------------------------------------
 300  CONTINUE
      ...
      IF (IER .GT. 0) IFLAG = 3
      RETURN
C----------------------- End of Subroutine SSPIOM ----------------------
      END
*DECK SATV
      SUBROUTINE SATV (NEQ, Y, SAVF, V, WGHT, FTEM, F, PSOL, Z, VTEM,
     1                WP, IWP, HL0, JPRE, IER, NPSL)
      EXTERNAL F, PSOL
      INTEGER NEQ, IWP, JPRE, IER, NPSL
      REAL Y, SAVF, V, WGHT, FTEM, Z, VTEM, WP, HL0
...
C Apply D-inverse to Z and return.
 ...
 90     Z(I) = Z(I)*WGHT(I)
      RETURN
C----------------------- End of Subroutine SATV ------------------------
      END
*DECK SORTHOG
      SUBROUTINE SORTHOG (VNEW, V, HES, N, LL, LDHES, KMP, SNORMW)
...
C-----------------------------------------------------------------------
      INTEGER I, I0
      REAL ARG, SDOT, SNRM2, SUMDSQ, TEM, VNRM
C
...
C
      RETURN
C----------------------- End of Subroutine SORTHOG ---------------------
      END
*DECK SSPIGMR
      SUBROUTINE SSPIGMR (NEQ, TN, Y, SAVF, B, WGHT, N, MAXL, MAXLP1,
     ...
     2  LGMR, WP, IWP, WK, DL, IFLAG)
      ...
      REAL TN,Y,SAVF,B,WGHT,DELTA,HL0,X,V,HES,Q,WP,WK,DL
      DIMENSION NEQ(*), Y(*), SAVF(*), B(*), WGHT(*), X(*), V(N,*),
...
C                (B is the same as V(*,MAXL+1) in the call to SSPIGMR.)
C
...
C        MNEWT = Newton iteration counter (.ge. 0).
C
C           WK = real work array used by routine SATV and PSOL.
C
...
C-----------------------------------------------------------------------
      ...
      REAL BNRM,BNRM0,C,DLNRM,PROD,RHO,S,SNORMW,SNRM2,TEM
C
      IFLAG = 0
      LGMR = 0
      NPSL = 0
...
C-----------------------------------------------------------------------
      ...
      DO 90 LL = 1,MAXL
        LGMR = LL
C-----------------------------------------------------------------------
C Call routine SATV to compute VNEW = Abar*v(ll), where Abar is
C the matrix A with scaling and inverse preconditioner factors applied.
...
C
      RETURN
C----------------------- End of Subroutine SSPIGMR ---------------------
      END
*DECK SPCG
      SUBROUTINE SPCG (NEQ, TN, Y, SAVF, R, WGHT, N, MAXL, DELTA, HL0,
     1 JPRE, MNEWT, F, PSOL, NPSL, X, P, W, Z, LPCG, WP, IWP, WK, IFLAG)
      EXTERNAL F, PSOL
      INTEGER NEQ, N, MAXL, JPRE, MNEWT, NPSL, LPCG, IWP, IFLAG
      REAL TN,Y,SAVF,R,WGHT,DELTA,HL0,X,P,W,Z,WP,WK
...
C        MNEWT = Newton iteration counter (.ge. 0).
C
C           WK = real work array used by routine SATP.
C
C           WP = real work array used by preconditioner PSOL.
C
C          IWP = integer work array used by preconditioner PSOL.
...
C-----------------------------------------------------------------------
      INTEGER I, IER
      REAL ALPHA,BETA,BNRM,PTW,RNRM,SDOT,SVNORM,ZTR,ZTR0
C
      IFLAG = 0
      NPSL = 0
      LPCG = 0
...
C Loop point for PCG iterations. ---------------------------------------
 ...
 40   CONTINUE
      ...
      GO TO 70
 50   CONTINUE
      IF (ZTR0 .EQ. 0.0E0) GO TO 200
      BETA = ZTR/ZTR0
      DO 60 I = 1,N
 60     P(I) = Z(I) + BETA*P(I)
 70   CONTINUE
C-----------------------------------------------------------------------
C  Call SATP to compute A*p and return the answer in W.
C-----------------------------------------------------------------------
      CALL SATP (NEQ, Y, SAVF, P, WGHT, HL0, WK, F, W)
C
      PTW = SDOT (N, P, 1, W, 1)
      IF (PTW .EQ. 0.0E0) GO TO 200
      ALPHA = ZTR/PTW
      CALL SAXPY (N, ALPHA, P, 1, X, 1)
      ALPHA = -ALPHA
      CALL SAXPY (N, ALPHA, W, 1, R, 1)
      RNRM = SVNORM (N, R, WGHT)
      IF (RNRM .LE. DELTA) RETURN
      IF (LPCG .LT. MAXL) GO TO 30
      IFLAG = 2
      IF (RNRM .LE. 1.0E0) IFLAG = 1
...
C-----------------------------------------------------------------------
 200  CONTINUE
      IFLAG = 4
      RETURN
C----------------------- End of Subroutine SPCG ------------------------
      END
*DECK SPCGS
      SUBROUTINE SPCGS (NEQ, TN, Y, SAVF, R, WGHT, N, MAXL, DELTA, HL0,
     1 JPRE, MNEWT, F, PSOL, NPSL, X, P, W, Z, LPCG, WP, IWP, WK, IFLAG)
      EXTERNAL F, PSOL
      INTEGER NEQ, N, MAXL, JPRE, MNEWT, NPSL, LPCG, IWP, IFLAG
      REAL TN,Y,SAVF,R,WGHT,DELTA,HL0,X,P,W,Z,WP,WK
...
C-----------------------------------------------------------------------
      INTEGER I, IER
      REAL ALPHA, BETA, BNRM, PTW, RNRM, SVNORM, ZTR, ZTR0
C
      IFLAG = 0
      NPSL = 0
      LPCG = 0
...
C-----------------------------------------------------------------------
 200  CONTINUE
      IFLAG = 4
      RETURN
C----------------------- End of Subroutine SPCGS -----------------------
      END
*DECK SATP
      SUBROUTINE SATP (NEQ, Y, SAVF, P, WGHT, HL0, WK, F, W)
      EXTERNAL F
      INTEGER NEQ
      REAL Y, SAVF, P, WGHT, HL0, WK, W
...
C-----------------------------------------------------------------------
      INTEGER I
      REAL FAC, PNRM, RPNRM, SVNORM
C
      PNRM = SVNORM (N, P, WGHT)
      RPNRM = 1.0E0/PNRM
      CALL SCOPY (N, Y, 1, W, 1)
      DO 20 I = 1,N
 20     Y(I) = W(I) + P(I)*RPNRM
      CALL F (NEQ, TN, Y, WK)
      NFE = NFE + 1
      CALL SCOPY (N, W, 1, Y, 1)
      FAC = HL0*PNRM
      DO 40 I = 1,N
 40     W(I) = P(I) - FAC*(WK(I) - SAVF(I))
      RETURN
C----------------------- End of Subroutine SATP ------------------------
      END
*DECK SUSOL
      SUBROUTINE SUSOL (NEQ, TN, Y, SAVF, B, WGHT, N, DELTA, HL0, MNEWT,
     1   PSOL, NPSL, X, WP, IWP, WK, IFLAG)
      EXTERNAL PSOL
      INTEGER NEQ, N, MNEWT, NPSL, IWP, IFLAG
      REAL TN, Y, SAVF, B, WGHT, DELTA, HL0, X, WP, WK
...
C-----------------------------------------------------------------------
 100  CONTINUE
      ...
      IF (IER .GT. 0) IFLAG = 3
      RETURN
C----------------------- End of Subroutine SUSOL -----------------------
      END
*DECK SSRCPK
      SUBROUTINE SSRCPK (RSAV, ISAV, JOB)
C-----------------------------------------------------------------------
C This routine saves or restores (depending on JOB) the contents of
C the Common blocks SLS001, SLPK01, which are used
C internally by the SLSODPK solver.
C
...
C-----------------------------------------------------------------------
      ...
      COMMON /SLS001/ RLS(218), ILS(37)
      COMMON /SLPK01/ RLSP(4), ILSP(13)
      DATA LENRLS/218/, LENILS/37/, LENRLP/4/, LENILP/13/
C
      IF (JOB .EQ. 2) GO TO 100
      CALL SCOPY (LENRLS, RLS, 1, RSAV, 1)
      CALL SCOPY (LENRLP, RLSP, 1, RSAV(LENRLS+1), 1)
      DO 20 I = 1,LENILS
 20     ISAV(I) = ILS(I)
      DO 40 I = 1,LENILP
...
C
 100  CONTINUE
      CALL SCOPY (LENRLS, RSAV, 1, RLS, 1)
      CALL SCOPY (LENRLP, RSAV(LENRLS+1), 1, RLSP, 1)
      DO 120 I = 1,LENILS
 120     ILS(I) = ISAV(I)
      DO 140 I = 1,LENILP
 140     ILSP(I) = ISAV(LENILS+I)
      RETURN
C----------------------- End of Subroutine SSRCPK ----------------------
      END
*DECK SHEFA
...
C----------------------- End of Subroutine SHELS -----------------------
      END
*DECK SLHIN
      SUBROUTINE SLHIN (NEQ, N, T0, Y0, YDOT, F, TOUT, UROUND,
     1   EWT, ITOL, ATOL, Y, TEMP, H0, NITER, IER)
      EXTERNAL F
      REAL T0, Y0, YDOT, TOUT, UROUND, EWT, ATOL, Y,
     1   TEMP, H0
...
C
C Subroutines called by SLHIN: F, SCOPY
C Function routines called by SLHIN: SVNORM
C-----------------------------------------------------------------------
...
C
C Communication with SLHIN is done with the following variables:
C
...
C
     ...
     1     HUB, HUN, PT1, T1, TDIST, TROUND, TWO, SVNORM, YDDNRM
      INTEGER I, ITER
C-----------------------------------------------------------------------
...
C Error return for TOUT - T0 too small. --------------------------------
 100  IER = -1
      RETURN
C----------------------- End of Subroutine SLHIN -----------------------
      END
*DECK SSTOKA
      SUBROUTINE SSTOKA (NEQ, Y, YH, NYH, YH1, EWT, SAVF, SAVX, ACOR,
     1   WM, IWM, F, JAC, PSOL)
      EXTERNAL F, JAC, PSOL
      INTEGER NEQ, NYH, IWM
      REAL Y, YH, YH1, EWT, SAVF, SAVX, ACOR, WM
     ...
     2   NNI, NLI, NPS, NCFN, NCFL
C-----------------------------------------------------------------------
C SSTOKA performs one step of the integration of an initial value
C problem for a system of Ordinary Differential Equations.
C
C This routine was derived from Subroutine SSTODPK in the SLSODPK
C package by the addition of automatic functional/Newton iteration
C switching and logic for re-use of Jacobian data.
C-----------------------------------------------------------------------
C Note: SSTOKA is independent of the value of the iteration method
C indicator MITER, when this is .ne. 0, and hence is independent
C of the type of chord method used, or the Jacobian structure.
C Communication with SSTOKA is done with the following variables:
C
C NEQ    = integer array containing problem size in NEQ(1), and
C          passed as the NEQ argument in all calls to F and JAC.
C Y      = an array of length .ge. N used as the Y argument in
...
C WM,IWM = real and integer work arrays associated with matrix
C          operations in chord iteration (MITER .ne. 0).
C CCMAX  = maximum relative change in H*EL0 before SSETPK is called.
C H      = the step size to be attempted on the next step.
C          H is altered by the error control algorithm during the
C          problem.  H can be either positive or negative, but its
C          sign must remain constant throughout the problem.
...
C              -1  the requested error could not be achieved.
C              -2  corrector convergence could not be achieved.
C              -3  fatal error in SSETPK or SSOLPK.
C          A return with KFLAG = -1 or -2 means either
C          ABS(H) = HMIN or 10 consecutive failures occurred.
C          On a return with KFLAG negative, the values of TN and
C          the YH array are as of the beginning of the last
...
C MAXORD = the maximum order of integration method to be allowed.
C MAXCOR = the maximum number of corrector iterations allowed.
C MSBP   = maximum number of steps between SSETPK calls (MITER .gt. 0).
C MXNCF  = maximum number of convergence failures allowed.
C METH/MITER = the method flags.  See description in driver.
C N      = the number of first-order differential equations.
C-----------------------------------------------------------------------
      INTEGER I, I1, IREDO, IRET, J, JB, JOK, M, NCF, NEWQ, NSLOW
      REAL DCON, DDN, DEL, DELP, DRC, DSM, DUP, EXDN, EXSM,
     1   EXUP, SFNORM, R, RH, RHDN, RHSM, RHUP, ROC, STIFF, TOLD, SVNORM
C
      KFLAG = 0
      TOLD = TN
      NCF = 0
...
C multiplying the YH array by the Pascal triangle matrix.
C The flag IPUP is set according to whether matrix data is involved
C (NEWT .gt. 0 .and. JACFLG .ne. 0) or not, to trigger a call to SSETPK.
C IPUP is set to MITER when RC differs from 1 by more than CCMAX,
C and at least every MSBP steps, when JACFLG = 1.
C RC is the ratio of new to old values of the coefficient  H*EL(1).
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
C If indicated, SSETPK is called to update any matrix data needed,
C before starting the corrector iteration.
...
C-----------------------------------------------------------------------
      ...
      IF (ICF .EQ. 1 .AND. DRC .LT. 0.2E0) JOK = -1
      IF (ICF .EQ. 2) JOK = -1
      IF (JOK .EQ. -1) THEN
        NSLJ = NST
        NJEV = NJEV + 1
        ENDIF
      CALL SSETPK (NEQ, Y, YH1, EWT, ACOR, SAVF, JOK, WM, IWM, F, JAC)
      IPUP = 0
      RC = 1.0E0
      DRC = 0.0E0
      NSLP = NST
...
C-----------------------------------------------------------------------
 350  DO 360 I = 1,N
 360    SAVX(I) = H*SAVF(I) - (YH(I,2) + ACOR(I))
      SFNORM = SVNORM (N, SAVX, EWT)
      CALL SSOLPK (NEQ, Y, SAVF, SAVX, EWT, WM, IWM, F, PSOL)
      IF (IERSL .LT. 0) GO TO 430
      IF (IERSL .GT. 0) GO TO 410
      DEL = SVNORM (N, SAVX, EWT)
      IF (DEL .GT. 1.0E-8) STIFF = MAX(STIFF, SFNORM/DEL)
      DO 380 I = 1,N
        ACOR(I) = ACOR(I) + SAVX(I)
 380    Y(I) = YH(I,1) + EL(1)*ACOR(I)
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
 400  IF (M .NE. 0) THEN
        ROC = MAX(0.05E0, DEL/DELP)
        CRATE = MAX(0.2E0*CRATE,ROC)
        ENDIF
      DCON = DEL*MIN(1.0E0,1.5E0*CRATE)/EPCON
      IF (DCON .LE. 1.0E0) GO TO 450
      M = M + 1
      IF (M .EQ. MAXCOR) GO TO 410
      IF (M .GE. 2 .AND. DEL .GT. 2.0E0*DELP) GO TO 410
      IF (ROC .GT. 10.0E0) GO TO 410
      IF (ROC .GT. 0.8E0) NSLOW = NSLOW + 1
...
C-----------------------------------------------------------------------
 450  JCUR = 0
      IF (NEWT .GT. 0) STIFR = 0.5E0*(STIFR + STIFF)
      IF (M .EQ. 0) DSM = DEL/TESCO(2,NQ)
      IF (M .GT. 0) DSM = SVNORM (N, ACOR, EWT)/TESCO(2,NQ)
      IF (DSM .GT. 1.0E0) GO TO 500
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      ...
      IF (NEWT .GT. 0 .AND. STIFR .LT. 1.5E0) NEWT = 0
      HU = H
      NQU = NQ
      DO 470 J = 1,L
        DO 470 I = 1,N
...
C-----------------------------------------------------------------------
 ...
 720  HOLD = H
      JSTART = 1
      RETURN
C----------------------- End of Subroutine SSTOKA ----------------------
      END
*DECK SSETPK
      SUBROUTINE SSETPK (NEQ, Y, YSV, EWT, FTEM, SAVF, JOK, WM, IWM,
     1                  F, JAC)
      EXTERNAL F, JAC
      INTEGER NEQ, JOK, IWM
      REAL Y, YSV, EWT, FTEM, SAVF, WM
     ...
     2   NNI, NLI, NPS, NCFN, NCFL
C-----------------------------------------------------------------------
C SSETPK is called by SSTOKA to interface with the user-supplied
C routine JAC, to compute and process relevant parts of
C the matrix P = I - H*EL(1)*J , where J is the Jacobian df/dy,
C as need for preconditioning matrix operations later.
C
C In addition to variables described previously, communication
C with SSETPK uses the following:
C Y     = array containing predicted values on entry.
C YSV   = array containing predicted y, to be saved (YH1 in SSTOKA).
C FTEM  = work array of length N (ACOR in SSTOKA).
C SAVF  = array containing f evaluated at predicted y.
...
C
     ...
     1   WM(LOCWP), IWM(LOCIWP), IER)
      ...
      IERPJ = 1
      RETURN
C----------------------- End of Subroutine SSETPK ----------------------
      END
*DECK SSRCKR
      SUBROUTINE SSRCKR (RSAV, ISAV, JOB)
C-----------------------------------------------------------------------
C This routine saves or restores (depending on JOB) the contents of
C the Common blocks SLS001, SLS002, SLSR01, SLPK01, which
C are used internally by the SLSODKR solver.
C
...
C-----------------------------------------------------------------------
      ...
      REAL RSAV,   RLS, RLS2, RLSR, RLSP
      DIMENSION RSAV(*), ISAV(*)
      SAVE LENRLS, LENILS, LENRLP, LENILP, LENRLR, LENILR
      COMMON /SLS001/ RLS(218), ILS(37)
      COMMON /SLS002/ RLS2, ILS2(4)
      COMMON /SLSR01/ RLSR(5), ILSR(9)
      COMMON /SLPK01/ RLSP(4), ILSP(13)
      DATA LENRLS/218/, LENILS/37/, LENRLP/4/, LENILP/13/
      DATA LENRLR/5/, LENILR/9/
C
      ...
      CALL SCOPY (LENRLS, RLS, 1, RSAV, 1)
      RSAV(LENRLS+1) = RLS2
      CALL SCOPY (LENRLR, RLSR, 1, RSAV(LENRLS+2), 1)
      CALL SCOPY (LENRLP, RLSP, 1, RSAV(LENRLS+LENRLR+2), 1)
      DO 20 I = 1,LENILS
 20     ISAV(I) = ILS(I)
...
C
 100  CONTINUE
      CALL SCOPY (LENRLS, RSAV, 1, RLS, 1)
      RLS2 = RSAV(LENRLS+1)
      CALL SCOPY (LENRLR, RSAV(LENRLS+2), 1, RLSR, 1)
      CALL SCOPY (LENRLP, RSAV(LENRLS+LENRLR+2), 1, RLSP, 1)
      DO 120 I = 1,LENILS
 120    ILS(I) = ISAV(I)
 ...
 140    ILSP(I) = ISAV(IOFF+I)
      RETURN
C----------------------- End of Subroutine SSRCKR ----------------------
      END
*DECK SAINVG
      SUBROUTINE SAINVG (RES, ADDA, NEQ, T, Y, YDOT, MITER,
     1                   ML, MU, PW, IPVT, IER )
      EXTERNAL RES, ADDA
...
C     A * YDOT = g(t,y)
C when A is nonsingular.  It is called by SLSODI for
C initialization only, when ISTATE = 0 .
C SAINVG returns an error flag IER:
C   IER  =  0  means SAINVG was successful.
...
C
  ...
  120 CALL SGBSL ( PW, NROWPW, NEQ, ML, MU, IPVT, YDOT, 0 )
      RETURN
C----------------------- End of Subroutine SAINVG ----------------------
      END
*DECK SSTODI
      SUBROUTINE SSTODI (NEQ, Y, YH, NYH, YH1, EWT, SAVF, SAVR,
     1   ACOR, WM, IWM, RES, ADDA, JAC, PJAC, SLVS )
      EXTERNAL RES, ADDA, JAC, PJAC, SLVS
      INTEGER NEQ, NYH, IWM
      REAL Y, YH, YH1, EWT, SAVF, SAVR, ACOR, WM
      DIMENSION NEQ(*), Y(*), YH(NYH,*), YH1(*), EWT(*), SAVF(*),
     1   SAVR(*), ACOR(*), WM(*), IWM(*)
      INTEGER IOWND, IALTH, IPUP, LMAX, MEO, NQNYH, NSLP,
     1   ICF, IERPJ, IERSL, JCUR, JSTART, KFLAG, L,
     ...
     5   MAXORD, MAXCOR, MSBP, MXNCF, N, NQ, NST, NFE, NJE, NQU
      ...
      REAL DCON, DDN, DEL, DELP, DSM, DUP,
     1   ELJH, EL1H, EXDN, EXSM, EXUP,
     2   R, RH, RHDN, RHSM, RHUP, TOLD, SVNORM
C-----------------------------------------------------------------------
C SSTODI performs one step of the integration of an initial value
C problem for a system of Ordinary Differential Equations.
C Note: SSTODI is independent of the value of the iteration method
C indicator MITER, and hence is independent
C of the type of chord method used, or the Jacobian structure.
C Communication with SSTODI is done with the following variables:
C
C NEQ    = integer array containing problem size in NEQ(1), and
...
C rate constant is stored in CRATE, and this is used in the test.
C-----------------------------------------------------------------------
      IF (M .NE. 0) CRATE = MAX(0.2E0*CRATE,DEL/DELP)
      DCON = DEL*MIN(1.0E0,1.5E0*CRATE)/(TESCO(2,NQ)*CONIT)
      IF (DCON .LE. 1.0E0) GO TO 460
      M = M + 1
      IF (M .EQ. MAXCOR) GO TO 410
      IF (M .GE. 2 .AND. DEL .GT. 2.0E0*DELP) GO TO 410
      DELP = DEL
...
CDIR$ IVDEP
 ...
 445    CONTINUE
      ...
      IF (ABS(H) .LE. HMIN*1.00001E0) GO TO 450
...
C-----------------------------------------------------------------------
 460  JCUR = 0
      IF (M .EQ. 0) DSM = DEL/TESCO(2,NQ)
      IF (M .GT. 0) DSM = ABS(H) * SVNORM (N, ACOR, EWT)/TESCO(2,NQ)
      IF (DSM .GT. 1.0E0) GO TO 500
C-----------------------------------------------------------------------
C After a successful step, update the YH array.
C Consider changing H if IALTH = 1.  Otherwise decrease IALTH by 1.
...
C-----------------------------------------------------------------------
 520  RHUP = 0.0E0
      ...
      DO 530 I = 1,N
 530    SAVF(I) = ACOR(I) - YH(I,LMAX)
      DUP = ABS(H) * SVNORM (N, SAVF, EWT)/TESCO(3,NQ)
      EXUP = 1.0E0/(L+1)
      RHUP = 1.0E0/(1.4E0*DUP**EXUP + 0.0000014E0)
 540  EXSM = 1.0E0/L
      RHSM = 1.0E0/(1.2E0*DSM**EXSM + 0.0000012E0)
...
C-----------------------------------------------------------------------
 ...
 720  HOLD = H
      JSTART = 1
      RETURN
C----------------------- End of Subroutine SSTODI ----------------------
      END
*DECK SPREPJI
      SUBROUTINE SPREPJI (NEQ, Y, YH, NYH, EWT, RTEM, SAVR, S, WM, IWM,
     1   RES, JAC, ADDA)
      EXTERNAL RES, JAC, ADDA
      INTEGER NEQ, NYH, IWM
      REAL Y, YH, EWT, RTEM, SAVR, S, WM
     ...
     1   MBA, MBAND, MEB1, MEBAND, ML, ML3, MU
      REAL CON, FAC, HL0, R, SRUR, YI, YJ, YJJ
C-----------------------------------------------------------------------
C SPREPJI is called by SSTODI to compute and process the matrix
...
C matrix.  This is done by SGEFA if MITER = 1 or 2, and by
C SGBFA if MITER = 4 or 5.
C
C In addition to variables described previously, communication
C with SPREPJI uses the following:
C Y     = array containing predicted values on entry.
C RTEM  = work array of length N (ACOR in SSTODI).
C SAVR  = array used for output only.  On output it contains the
C         residual evaluated at current values of t and y.
C S     = array containing predicted values of dy/dt (SAVF in SSTODI).
C WM    = real work space for matrices.  On output it contains the
C         LU decomposition of P.
C         Storage of matrix elements starts at WM(3).
C         WM also contains the following matrix-related data:
...
C If MITER = 2, make N + 1 calls to RES to approximate J. --------------
 200  CONTINUE
      ...
      DO 230 J = 1,N
        YJ = Y(J)
        R = MAX(SRUR*ABS(YJ),0.01E0/EWT(J))
        Y(J) = Y(J) + R
        FAC = -HL0/R
...
C If MITER = 5, make ML + MU + 2 calls to RES to approximate J. --------
 500  CONTINUE
      ...
      DO 560 J = 1,MBA
        DO 530 I = J,N,MBAND
          YI = Y(I)
          R = MAX(SRUR*ABS(YI),0.01E0/EWT(I))
 530      Y(I) = Y(I) + R
        CALL RES ( NEQ, TN, Y, S, RTEM, IRES)
        NFE = NFE + 1
        IF (IRES .GT. 1) GO TO 600
        DO 550 JJ = J,N,MBAND
          Y(JJ) = YH(JJ,1)
          YJJ = Y(JJ)
          R = MAX(SRUR*ABS(YJJ),0.01E0/EWT(JJ))
          FAC = -HL0/R
          I1 = MAX(JJ-MU,1)
          I2 = MIN(JJ+ML,N)
          II = JJ*MEB1 - ML + 2
...
C Error return for IRES = 2 or IRES = 3 return from RES. ---------------
 600  IERPJ = IRES
      RETURN
C----------------------- End of Subroutine SPREPJI ---------------------
      END
*DECK SAIGBT
      SUBROUTINE SAIGBT (RES, ADDA, NEQ, T, Y, YDOT,
     1                   MB, NB, PW, IPVT, IER )
      EXTERNAL RES, ADDA
...
C of the vector YDOT satisfying
C     A * YDOT = g(t,y)
C when A is nonsingular.  It is called by SLSOIBT for
C initialization only, when ISTATE = 0 .
C SAIGBT returns an error flag IER:
C   IER  =  0  means SAIGBT was successful.
C   IER .ge. 2 means RES returned an error flag IRES = IER.
...
C-----------------------------------------------------------------------
 ...
 10     PW(I) = 0.0E0
      ...
      CALL SDECBT (MB, NB, PW, PW(LPB), PW(LPC), IPVT, IER)
      IF (IER .EQ. 0) GO TO 20
      IER = -IER
      RETURN
 20   CALL SSOLBT (MB, NB, PW, PW(LPB), PW(LPC), YDOT, IPVT)
      RETURN
C----------------------- End of Subroutine SAIGBT ----------------------
      END
*DECK SPJIBT
      SUBROUTINE SPJIBT (NEQ, Y, YH, NYH, EWT, RTEM, SAVR, S, WM, IWM,
     1   RES, JAC, ADDA)
      EXTERNAL RES, JAC, ADDA
      INTEGER NEQ, NYH, IWM
      REAL Y, YH, EWT, RTEM, SAVR, S, WM
     ...
     1   K, K1, LENP, LBLOX, LPB, LPC, MB, MBSQ, MWID, NB
      REAL CON, FAC, HL0, R, SRUR
C-----------------------------------------------------------------------
C SPJIBT is called by SSTODI to compute and process the matrix
C P = A - H*EL(1)*J , where J is an approximation to the Jacobian dr/dy,
...
C P is then subjected to LU decomposition by SDECBT in preparation
C for later solution of linear systems with P as coefficient matrix.
C
C In addition to variables described previously, communication
C with SPJIBT uses the following:
C Y     = array containing predicted values on entry.
C RTEM  = work array of length N (ACOR in SSTODI).
C SAVR  = array used for output only.  On output it contains the
C         residual evaluated at current values of t and y.
...
C         Increment Y(I) for group of column indices, and call RES. ----
          J1 = J+(K-1)*MB
          DO 210 I = J1,N,MWID
            R = MAX(SRUR*ABS(Y(I)),0.01E0/EWT(I))
            Y(I) = Y(I) + R
 210      CONTINUE
          CALL RES (NEQ, TN, Y, S, RTEM, IRES)
          NFE = NFE + 1
...
C Do LU decomposition on P. --------------------------------------------
      CALL SDECBT (MB, NB, WM(3), WM(LPB), WM(LPC), IWM(21), IER)
      IF (IER .NE. 0) IERPJ = 1
      RETURN
C Error return for IRES = 2 or IRES = 3 return from RES. ---------------
 600  IERPJ = IRES
      RETURN
C----------------------- End of Subroutine SPJIBT ----------------------
      END
*DECK SSLSBT
      SUBROUTINE SSLSBT (WM, IWM, X, TEM)
      INTEGER IWM
...
C routine and the SSOLBT routine for the solution of the linear system
C arising from chord iteration.
C Communication with SSLSBT uses the following variables:
...
C-----------------------------------------------------------------------
      ...
      LPB = 3 + LBLOX
      LPC = LPB + LBLOX
      CALL SSOLBT (MB, NB, WM(3), WM(LPB), WM(LPC), X, IWM(21))
      RETURN
C----------------------- End of Subroutine SSLSBT ----------------------
      END
*DECK SDECBT
      SUBROUTINE SDECBT (M, N, A, B, C, IP, IER)
...
C SDECBT uses block Gauss elimination and Subroutines SGEFA and SGESL
C for solution of blocks.  Partial pivoting is done within
C block-rows only.
C
C Note: this version uses LINPACK routines SGEFA/SGESL instead of
C of dec/sol for solution of blocks, and it uses the BLAS routine SDOT
...
C Use SSOLBT to solve the associated linear system.
C
C External routines required: SGEFA and SGESL (from LINPACK) and
C SDOT (from the BLAS, or Basic Linear Algebra package).
C-----------------------------------------------------------------------
...
C Process the first block-row. -----------------------------------------
      CALL SGEFA (A, M, M, IP, IER)
      K = 1
      IF (IER .NE. 0) GO TO 200
      DO 10 J = 1,M
        CALL SGESL (A, M, M, IP, B(1,J,1), 0)
        CALL SGESL (A, M, M, IP, C(1,J,1), 0)
 10     CONTINUE
C Adjust B(*,*,2). -----------------------------------------------------
      DO 40 J = 1,M
        DO 30 I = 1,M
          DP = SDOT (M, C(I,1,2), M, C(1,J,1), 1)
          B(I,J,2) = B(I,J,2) - DP
 30       CONTINUE
 40     CONTINUE
C Main loop.  Process block-rows 2 to N-1. -----------------------------
      DO 100 K = 2,NM1
        ...
        DO 70 J = 1,M
          DO 60 I = 1,M
            DP = SDOT (M, C(I,1,K), M, B(1,J,KM1), 1)
            A(I,J,K) = A(I,J,K) - DP
 60         CONTINUE
 70       CONTINUE
        CALL SGEFA (A(1,1,K), M, M, IP(1,K), IER)
        IF (IER .NE. 0) GO TO 200
        DO 80 J = 1,M
 80       CALL SGESL (A(1,1,K), M, M, IP(1,K), B(1,J,K), 0)
 100    CONTINUE
C Process last block-row and return. -----------------------------------
      DO 130 J = 1,M
        DO 120 I = 1,M
          DP = SDOT (M, B(I,1,N), M, B(1,J,NM2), 1)
          C(I,J,N) = C(I,J,N) - DP
 120      CONTINUE
 130    CONTINUE
      DO 160 J = 1,M
        DO 150 I = 1,M
          DP = SDOT (M, C(I,1,N), M, B(1,J,NM1), 1)
          A(I,J,N) = A(I,J,N) - DP
 150      CONTINUE
 160    CONTINUE
      CALL SGEFA (A(1,1,N), M, M, IP(1,N), IER)
      K = N
      IF (IER .NE. 0) GO TO 200
      RETURN
C Error returns. -------------------------------------------------------
 ...
 210  IER = -1
      RETURN
C----------------------- End of Subroutine SDECBT ----------------------
      END
*DECK SSOLBT
      SUBROUTINE SSOLBT (M, N, A, B, C, Y, IP)
      INTEGER M, N, IP(M,N)
      REAL A(M,M,N), B(M,M,N), C(M,M,N), Y(M,N)
C-----------------------------------------------------------------------
C Solution of block-tridiagonal linear system.
C Coefficient matrix must have been previously processed by SDECBT.
C M, N, A,B,C, and IP  must not have been changed since call to SDECBT.
C Written by A. C. Hindmarsh.
C Input:
C     M = order of each block.
C     N = number of blocks in each direction of matrix.
C A,B,C = M by M by N arrays containing block LU decomposition
C         of coefficient matrix from SDECBT.
C    IP = M by N integer array of pivot information from SDECBT.
...
C Backward solution sweep. ---------------------------------------------
 ...
 100    CONTINUE
      RETURN
C----------------------- End of Subroutine SSOLBT ----------------------
      END
*DECK SIPREPI
      SUBROUTINE SIPREPI (NEQ, Y, S, RWORK, IA, JA, IC, JC, IPFLAG,
     1   RES, JAC, ADDA)
      EXTERNAL RES, JAC, ADDA
...
C-----------------------------------------------------------------------
C This routine serves as an interface between the driver and
C Subroutine SPREPI.  Tasks performed here are:
C  * call SPREPI,
C  * reset the required WM segment length LENWK,
C  * move YH back to its final location (following WM in RWORK),
...
C IPFLAG is an output error indication flag.  IPFLAG = 0 if there was
C no trouble, and IPFLAG is the value of the SPREPI error flag IPPER
C if there was trouble in Subroutine SPREPI.
C-----------------------------------------------------------------------
      IPFLAG = 0
C Call SPREPI to do matrix preprocessing operations. -------------------
      CALL SPREPI (NEQ, Y, S, RWORK(LYH), RWORK(LSAVF), RWORK(LEWT),
     1   RWORK(LACOR), IA, JA, IC, JC, RWORK(LWM), RWORK(LWM), IPFLAG,
     2   RES, JAC, ADDA)
      LENWK = MAX(LREQ,LWMIN)
      IF (IPFLAG .LT. 0) RETURN
C If SPREPI was successful, move YH to end of required space for WM. ---
      LYHN = LWM + LENWK
      IF (LYHN .GT. LYH) RETURN
      LYHD = LYH - LYHN
      IF (LYHD .EQ. 0) GO TO 20
...
C If ISTATE = 1, move EWT (left) to its new position. ------------------
 ...
 40   LEWT = LEWTN
      RETURN
C----------------------- End of Subroutine SIPREPI ---------------------
      END
*DECK SPREPI
      SUBROUTINE SPREPI (NEQ, Y, S, YH, SAVR, EWT, RTEM, IA, JA, IC, JC,
     1                   WK, IWK, IPPER, RES, JAC, ADDA)
      EXTERNAL RES, JAC, ADDA
      INTEGER NEQ, IA, JA, IC, JC, IWK, IPPER
      REAL Y, S, YH, SAVR, EWT, RTEM, WK
...
C  * perform a symbolic LU factorization of the matrix, and
C  * set pointers for segments of the IWK/WK array.
C In addition to variables described previously, SPREPI uses the
C following for communication:
C YH     = the history array.  Only the first column, containing the
C          current Y vector, is used.  Used only if MOSS .ne. 0.
...
C A dummy call to RES allows user to create temporaries for use in JAC.
 ...
 75     WK(LENWK1+I) = 0.0E0
      ...
      DO 95 J = 1,N
        ...
        DO 90 I = 1,N
          LJFO = LENWK1 + I
          IF (WK(LJFO) .EQ. 0.0E0) GO TO 80
          WK(LJFO) = 0.0E0
          SAVR(I) = 0.0E0
          GO TO 85
 80       IF (SAVR(I) .EQ. 0.0E0) GO TO 90
          SAVR(I) = 0.0E0
...
C
 370  IPPER = -IER - 5
      LREQ = 2 + (2*N + 1)/LRAT
      RETURN
C----------------------- End of Subroutine SPREPI ----------------------
      END
*DECK SAINVGS
      SUBROUTINE SAINVGS (NEQ, T, Y, WK, IWK, TEM, YDOT, IER, RES, ADDA)
      EXTERNAL RES, ADDA
      INTEGER NEQ, IWK, IER
      INTEGER IPLOST, IESP, ISTATC, IYS, IBA, IBIAN, IBJAN, IBJGP,
     1   IPIAN, IPJAN, IPJGP, IPIGP, IPR, IPC, IPIC, IPISP, IPRSP, IPA,
...
C satisfying
C     A * YDOT = g(t,y)
C when A is nonsingular.  It is called by SLSODIS for initialization
C only, when ISTATE = 0.  The matrix A is subjected to LU
C decomposition in SCDRV.  Then the system A*YDOT = g(t,y) is solved
C in SCDRV.
C In addition to variables described previously, communication
C with SAINVGS uses the following:
...
C         are assumed to have identical locations.
C TEM   = vector of work space of length N (ACOR in SSTODI).
...
C       = 4  if insufficient storage for SCDRV (should not occur here).
C       = 5  if other error found in SCDRV (should not occur here).
C-----------------------------------------------------------------------
C
      DO 10 I = 1,NNZ
 10     WK(IBA+I) = 0.0E0
...
C Numerical factorization of matrix A. ---------------------------------
      CALL SCDRV (NEQ,IWK(IPR),IWK(IPC),IWK(IPIC),IWK(IPIAN),IWK(IPJAN),
...
C Solution of the linear system. ---------------------------------------
 50   CALL SCDRV (NEQ,IWK(IPR),IWK(IPC),IWK(IPIC),IWK(IPIAN),IWK(IPJAN),
     1  WK(IPA),YDOT,YDOT,NSP,IWK(IPISP),WK(IPRSP),IESP,4,IYS)
      IF (IYS .NE. 0) IER = 5
      RETURN
C----------------------- End of Subroutine SAINVGS ---------------------
      END
*DECK SPRJIS
      SUBROUTINE SPRJIS (NEQ, Y, YH, NYH, EWT, RTEM, SAVR, S, WK, IWK,
     1   RES, JAC, ADDA)
      EXTERNAL RES, JAC, ADDA
      INTEGER NEQ, NYH, IWK
      REAL Y, YH, EWT, RTEM, SAVR, S, WK
      DIMENSION NEQ(*), Y(*), YH(NYH,*), EWT(*), RTEM(*),
     1   S(*), SAVR(*), WK(*), IWK(*)
      INTEGER IOWND, IOWNS,
     1   ICF, IERPJ, IERSL, JCUR, JSTART, KFLAG, L,
     ...
     4   NSLJ, NGP, NLU, NNZ, NSP, NZL, NZU
      ...
      REAL CON, FAC, HL0, R, SRUR
C-----------------------------------------------------------------------
C SPRJIS is called to compute and process the matrix
C P = A - H*EL(1)*J, where J is an approximation to the Jacobian dr/dy,
...
C generate P.  The matrix P is subjected to LU decomposition in SCDRV.
C P and its LU decomposition are stored separately in WK.
C
C In addition to variables described previously, communication
C with SPRJIS uses the following:
C Y     = array containing predicted values on entry.
C RTEM  = work array of length N (ACOR in SSTODI).
...
C         =  1 if zero pivot found in SCDRV.
C         = IRES (= 2 or 3) if RES returned IRES = 2 or 3.
C         = -1 if insufficient storage for SCDRV (should not occur).
C         = -2 if other error found in SCDRV (should not occur here).
C JCUR  = output flag = 1 to indicate that the Jacobian matrix
C         (or approximation) is now current.
C This routine also uses other variables in Common.
...
C If MITER = 2, make NGP + 1 calls to RES to approximate J and P. ------
 200  CONTINUE
      ...
      DO 240 NG = 1,NGP
        ...
        DO 210 J = JMIN,JMAX
          JJ = IWK(IBJGP+J)
          R = MAX(SRUR*ABS(Y(JJ)),0.01E0/EWT(JJ))
 210      Y(JJ) = Y(JJ) + R
        CALL RES (NEQ,TN,Y,S,RTEM,IRES)
        NFE = NFE + 1
        IF (IRES .GT. 1) GO TO 600
