...
c
      ...
      a(3)=-61.0e0/90.0e0
      a(4)=56.0e0/25.0e0
      a(5)=-901.0e0/1525.0e0
      a(6)=-198.0e0/305.0e0

      b(1)=-91.0e0/125.0e0
      b(2)=7386.0e0/7625.0e0
      b(3)=10908.0e0/16775.0e0
      b(4)=6.0e0/55.0e0

      cloc(1)=-6.0e0/125.0e0
      cloc(2)=-24.0e0/7625.0e0
      cloc(3)=-972.0e0/16775.0e0
      cloc(4)=6.0e0/55.0e0
...
c
      uround = rumach()
      reps = amax1(reps,100.0e0*uround)
      e13=-1.0e0/3.0e0
      n5=4*n
      n7=6*n
      n11=10*n
...
c
      do 25 i=1,n
           work(i) = aeps(i) + reps*abs(work(nt+i))
 25   continue
...
c
      if( ( iflag . eq . -2 ) . or . ( efac . lt . 0.95e0 ) )  goto 200
c
c     step accepted
c
      ns=ns+1
      if(ityp.eq.1) then
                        ...
                        if ( stifg . lt . 2.0e0 ) ityp = 1
      endif

      hold = h
      hsmall = abs(hold)*1.0e-4
      nx=nt
      nt=nb
      nb=nx
c
...
c
 ...
 44   continue
      ...
      iflag=1
      if ( efac . lt . 1.1e0 ) then
                                  h = h*0.9e0
                             else
           if( ( nsred . le . 0 ) .or. (efac .lt. 2.0e0 )) goto 300
           hinc = amin1(efac,rock) * 0.9e0
           h = amin1(h*hinc,hmax,4.0e0*h)
      endif
...
c   step has to be reduced::::::::::::::::::::::::::::::::::
      ...
      h=amin1(amax1(0.1e0,rock),efac)*h*0.8e0
 210  gah=ga*h
c     write(6,212)x,h
...
c
 ...
 300  continue
      ...
      if( ityp .eq. 0) stifg = 0.5e0*(stifg + stiff)
...
c
      ...
      do 30 i=1,n
            diff=work(m12+i)
            sum=amax1(sum,abs(diff)/ewt(i))
            y(i)=y(i)+diff
 30   continue
      if (sum .eq. 0.0e0) return
      stiff = amax1(stiff, rsum/sum)
      roc = amax1(0.05e0,sum/sumold)
      if ( sum .lt. rer ) return
      sumold = sum
      if ( roc .lt. 0.8e0 ) goto 5
      if ( roc .gt. 10.0e0 ) goto 191
      if ( ir .gt. 0 ) goto 191
      ir = ir + 1
      goto 5
...
c
      ...
      k2=k1+n
      u38=u**0.375e0
      da=amax1(100.0e0*u*abs(a),u38)
      da=amax1(amin1(u38*abs(a),b-a),da)
 ...
 10   continue
      ...
      dmax=0.0e0
      do 20 i=1,n
         dmax=amax1(dmax,abs(work(k2+i)-work(k0+i))/ewt(i))
 20   continue
      ep=(1.0e0/cp)**(1.0e0/(p+1.0e0))
      dy2=dmax/da
      dy2=sqrt(dy2)
      h1=(b-a)/100.0e0
 ...
 35   continue
      ...
      dmax=0.0e0
      do 40 i=1,n
         dmax=amax1(dmax,abs(work(k1+i)-work(k0+i))/ewt(i))
 40   continue
      dy3=dmax/da
      h2=(b-a)/100.0e0
      dy3=sqrt(dy3)
      if(dy3.gt.da) h2=ep/dy3
      h=amin1(h1,h2,b-a)*0.1e0
      return
      end
...
c----------------------- end of subroutine pkset -----------------------
     ...
     1   apre, ipre, x, b, wk, iwk, iersl)
      external f, psol
      real tn, y, gah, ewt, savf, apre, x, b, wk
...
c----------------------- end of subroutine solpk -----------------------
     ...
     2  lgmr, coef, apre, ipre, wks, dl, iflag)
      ...
      real tn, y, savf, b, wght, delta, gah,
...
c-----------------------------------------------------------------------
 ...
 25     x(i) = 0.0e0
      return
 30   continue
c apply inverse of left preconditioner to v(*,1). ----------------------
      ier = 0
...
c since the vector x was used as work space in routine orthog and since
...
c-----------------------------------------------------------------------
 ...
 210    coef(k) = 0.0e0
      coef(1) = bnrm
      call shels (hes, maxlp1, ll, q, coef)
      do 220 k = 1,n
 220    x(k) = 0.0e0
      do 230 i = 1,ll
        call saxpy (n, coef(i), v(1,i), 1, x, 1)
 230    continue
      do 240 i = 1,n
 240    x(i) = x(i)*wght(i)
...
c----------------------- end of subroutine spigmr ----------------------
     ...
     1                apre, ipre, gah, jpre, ier, npsl)
      ...
      real tn, y, savf, v, wght, ftem, z, vtem, apre, gah
