...
logical, parameter :: threeshl_lt_enabled = .false.
...
logical, public :: threeshl_use_nlow=.false.
...
logical, save :: warned=.false.
 ...
 if (threeshl_use_nlow) then
  if (.not. warned) then
   write (threeshl_errunit, *) "WARNING: QCD corrected widths requested, but support is not compiled in!"
   warned = .true.
  end if
 end if
 end if
