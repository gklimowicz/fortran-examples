...
!!!GluGlu to (shiggs,phiggs)
  complex(kind=default), dimension(dimh0), public, save :: g_glugluh0
  complex(kind=default), dimension(dimA0), public, save :: g_gluglua0
...
contains
  subroutine import_from_whizard (par_array, scheme)
    real(default), dimension(168), intent(in) :: par_array
    ...
    par%v        = par_array(165)
    par%cw       = par_array(166)
    par%sw       = par_array(167)
    par%ee       = par_array(168)
    mass(1:73) = 0
    width(1:73) = 0
    mass(3) = par%ms
    mass(4) = par%mc
...
!!!!!!!!!!!!!!!!!!!!!!!
   do shiggs = 1,dimh0
      g_glugluh0(shiggs) = 0.0d0
      !! top and bottom in the loop
      do i = 3,3
         g_glugluh0(shiggs) = &
              g_glugluh0(shiggs) - gs**2/(8.0 * pi**2 * vev) * (&
              mix_h0(shiggs,2)/sinbe * loop_factor(mass(2*i), h0mass(shiggs),'ferh0')&
              + mix_h0(shiggs,1)/cosbe * loop_factor(mass(2*i-1), h0mass(shiggs),'ferh0')&
                 )
      enddo
      !!squarks in the loop
      do j = 1,3
         g_glugluh0(shiggs) = &
              g_glugluh0(shiggs) + gs**2/(8.0 * pi**2 * vev) * (&
              (vev / mass(39 + 2*j)**2) * g_h0_sd1sd1(shiggs,j) * loop_factor(mass(39 + 2*j), h0mass(shiggs),'squh0')&
              +(vev / mass(45 + 2*j)**2) * g_h0_sd2sd2(shiggs,j) * loop_factor(mass(45 + 2*j), h0mass(shiggs),'squh0')&
              +(vev / mass(40 + 2*j)**2) * g_h0_su1su1(shiggs,j) * loop_factor(mass(40 + 2*j), h0mass(shiggs),'squh0')&
              +(vev / mass(46 + 2*j)**2) * g_h0_su2su2(shiggs,j) * loop_factor(mass(46 + 2*j), h0mass(shiggs),'squh0')&
              )
      enddo
   end do
   g_glugluh0=-g_glugluh0
!!!
   do phiggs = 1,dimA0
      g_glugluA0(phiggs) = 0.0d0
      do i = 1,3
         g_glugluA0(phiggs) =  g_glugluA0(phiggs)  -&
              gs**2/(8.0* pi**2 * vev) * ( &
              mix_A0(phiggs,2)/sinbe * loop_factor(mass(2*i), A0mass(phiggs),'ferA0') &
              +   mix_A0(phiggs,1)/cosbe * loop_factor(mass(2*i-1), A0mass(phiggs),'ferA0') & 
              )
         
      enddo
   enddo
   g_glugluA0=-g_glugluA0
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!! auxilliary functions to build up the glugluhiggs couplings
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 real(kind=default) function A0mass(i)
   integer, intent(in) :: i  
   if (i.eq.1) then 
      A0mass = mass(36)
   elseif (i.eq.2) then
      ...
      A0mass = 0.00001_default
   endif
 end function A0mass
       
 real(kind=default) function h0mass(i)
   integer, intent(in) :: i
   if (i.eq.1) then 
      h0mass = mass(25)
   elseif (i.eq.2) then
   ...
   elseif (i.eq.3) then
      ...
      h0mass = 0.00001_default
   endif   
 end function h0mass

 complex(kind=default) function loop_factor(mloop, mh, type)
...
end module parameters_nmssm_hgg
