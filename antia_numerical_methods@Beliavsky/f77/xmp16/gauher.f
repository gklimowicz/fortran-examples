!     PROGRAM TO CALCULATE WEIGHTS AND ABSCISSAS OF GAUSS-HERMITE QUADRATURE FORMULAS
...
51    FORMAT(5X,'IER =',I4,'    N =',I4/9X,'ABSCISSAS',30X,'WEIGHTS'/
     1       (1PD41.33,5X,D41.33))
...
100   PRINT *,'TYPE  NT=NO. OF POINTS IN QUADRATURE FORMULA'
      ...
      CALL GAUHER(NT,W,X,IER,WK)
      WRITE(6,51) IER,NT,(X(I),W(I),I=1,NT)

!     USE THE WEIGHTS AND ABSCISSAS TO EVALUATE THE INTEGRAL OF X**N FOR TESTING
!     Since the magnitude of terms and the integrals increase with N the absolute
!     error will increase with N. However, since integral vanishes for odd N it
!     is not possible to use relative error. Thus the maximum error printed out
!     at the end will increase with NT.
