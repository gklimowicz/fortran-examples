!     PROGRAM TO CALCULATE MULTIPLE INTEGRALS OVER A HYPER-RECTANGLE IN
!     N DIMENSIONS USING PRODUCT GAUSS FORMULAS OR COMPOSITE MONOMIAL RULES
!     OR MONTE CARLO METHOD OR THE METHOD BASED ON EQUIDISTRIBUTED SEQUENCES
...
!     EXAMPLE 6.15 : INTEGRAL I3
 
51    FORMAT('   IER =',I4,5X,'IT =',I2,5X,'NO. OF FUNCTION',
     1       ' EVALUATIONS =',I8)
52    FORMAT('  INTEGRAL OVER',I3,' DIMENSIONS =',1PE14.6,5X,
     1       'ERROR ESTIMATE =',E14.6)
53    FORMAT('   IER =',I4,5X,'IT =',I2,5X,'M =',I2,5X,
     1       'NO. OF FUNCTION EVALUATIONS =',I8)
54    FORMAT('  INTEGRAL OVER',I3,' DIMENSIONS  S1 =',1PE14.6,5X,
     1       'S2 =',E14.6/9X,'ERROR ESTIMATE =',E14.6)
...
500   CONTINUE
      ...
      MAXPT=1000000 
      IF(IT.EQ.1) THEN 
        CALL MULINT(A,B,N,M,INT,FN,RI,REPS,AEPS,DIF,IER,NUM,MAXPT)
        WRITE(6,51) IER,IT,NUM
        WRITE(6,52) N,RI,DIF
      ELSE IF(IT.EQ.2) THEN
        PRINT *,'TYPE M=1,3 OR 5=DEGREE OF MONOMIAL RULE'
        READ *,MM
        CALL STRINT(A,B,N,MM,INT,FN,RI,REPS,AEPS,DIF,IER,NUM,MAXPT)
        WRITE(6,53) IER,IT,MM,NUM
        WRITE(6,52) N,RI,DIF
      ELSE IF(IT.EQ.3) THEN
        CALL MCARLO(A,B,N,MAXPT,FN,RI,REPS,AEPS,DIF,NUM,IER)
        WRITE(6,51) IER,IT,NUM
        WRITE(6,52) N,RI,DIF
      ELSE
        CALL EQUIDS(A,B,N,MAXPT,FN,S1,S2,REPS,AEPS,DIF,NUM,IER)
        WRITE(6,51) IER,IT,NUM
