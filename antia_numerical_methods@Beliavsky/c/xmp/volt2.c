/*	Solve nonlinear Volterra equation of second kind using Simpson's rule */
...
double fg(double x);
double fker(double x, double t, double f);
...
main()
{
	...
	double hh, x[200], f[200],tn,reps,t0,fi;

	reps=1.e-8;
	t0=0.0; 
	for(i1=0; i1<99; ++i1) {
		printf("type n = no. of steps,   hh = step size \n");
		printf("                      (quits when n<=0)\n");
		scanf(" %d %le",&n,&hh);
		if(n<=0) return 0;

		i=volt2(n,t0,hh,f,x,fg,fker,reps);
		printf(" ier = %d   no. of steps =  %d    step size = %e\n",i,n,hh);
		printf("   x          solution     exact solution\n");
		for(i=0; i<n; i +=5) printf(" %e %e  %e\n",x[i],f[i],exp(x[i]*x[i]));
...
{
	return -exp(x*x)+x-x*exp(0.5*x*x);
}
