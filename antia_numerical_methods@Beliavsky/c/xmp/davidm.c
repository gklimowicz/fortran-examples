...
main()
{
	...
	double x1, x2, root, reps, aeps, d2f, fx;

/*	Example 8.3 */

	aeps=1.e-8; reps=1.e-7;
	for(i1=0; i1<99; ++i1) {
		printf("type x1,x2 = initial approximations    (quits when x1=x2)\n");
		scanf(" %le %le", &x1, &x2);
		if(x1==x2) return 0;
		printf(" Initial approximations = %e  %e \n",x1,x2);

		i=davidm(&x1,&x2,&fx,&d2f,reps,aeps,fun);
		printf(" ier = %d  minimiser = %e    minimum = %e  f''(x) = %e\n", i,x2,fx,d2f);
	}
	return;
}
...
{
	*df=(0.1+2.0*x-10.0*x*x)*exp(-10.0*x);
