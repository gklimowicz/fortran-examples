/*	To solve initial value problem in ordinary differential equations
	using multi-step method with adaptive step size control */
...
/*	Exercise 12.34 : Bessel's equation for n=0 */

	id=4; n=2; nmax=100000; reps=1.e-7;
	printf("type  id=0/1 for Adams/ Gear,  t0=initial time,  hh=initial step size\n");
	scanf(" %d %le %le",&id,&t0,&hh);
	printf("   t0 = %e   initial step-size = %e   id = %d \n",t0,hh,id);

/*	To tackle the singularity at t=0 start the integration from t0>0
	and use the Maclaurin series to find initial values */
	x[0]=1.0; x[1]=0.0; t1=1.0; xt=t0*t0/4.0;
	for(i=1; i<=20; ++i) {
		t1=-t1*xt/(i*i);
		x[0]=x[0]+t1;
		x[1]=x[1]+2.*i*t1/t0;
		if(fabs(t1)<1.e-15) break;
	}

	m=0; iflg=0;
/*	Type successive values of t at which solution is required */
	for(i1=0; i1<99; ++i1) {
		printf("type  tn = final value of time    (quits when tn<-10)\n");
		scanf(" %le", &tn);
		if(tn<-10.0) return 0;

		i=mstep(n,x,dx,dif,&hh,&t0,tn,y,reps,&np,&nmax,&iflg,id,&wk[0]);
		printf(" ier = %d    no. of function evaluations =  %d    t = %e    step size = %e \n",i,np,tn,hh);
		printf(" solution : ");
		for(i=0; i<n; ++i) printf(" %e ",y[i]);
		printf(" \n");
