/*	To integrate  f(x)/sqrt(x)  over (0,a]  */
...
main()
{
	...
	double a1, xu, rint, reps, aeps, df, fb[20];
...
/*	Exercise 6.14 */
	...
	for(i1=0; i1<99; ++i1) {
		...
		i=gausq2(&rint,xu,&a1,reps,aeps,&df,fun,f2,&nuse);
		printf(" ier = %d    no. of function evaluations = %d   a = %e   a1 = %e \n", i,nuse,xu,a1);
		printf(" integral = %e   estimated error = %e  \n", rint, df);
	}
	return;
}

/*	The integrand for gauss */
...
/*	The integrand*sqrt(x) for use with gausq */
