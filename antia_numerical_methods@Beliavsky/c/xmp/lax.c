/*	To solve hyperbolic equations using Lax-Wendroff method */

#include <stdio.h>
#include <math.h>

void flux(int n, double x, double t, double u[], double f[]);
void bc(int ib, int n, double x, double t, int iw[], double a[]);
void fic(int n, double x, double t, double u[]);
...
main()
{
	...
	double hh, x[100], wt[100],f[100][10],fc[100],tn,erc,reps,t0,fi;
	double x0,xn,dt;

/*	Example 14.3 */

	id=10;
	x0=0.0; xn=0.5; iflg=0; m=2;
	...
	for(i1=0; i1<99; ++i1) {
		...
		i=lax(m,&t0,dt,x0,xn,nt,nx,x,&f[0][0],id,flux,bc,fic,&iflg);
		printf(" ier = %d  nx =  %d  nt = %d  t = %e  step length = %e \n",i,nx,nt,t0,dt);
		printf("     x         solution \n");
...
/*	The hyperbolic equation in flux conservation form */
...
void bc(int ib, int n, double x, double t, int ic[], double a[])

{
	int i;
	for(i=0; i<3*n; ++i) a[i]=0.0;

	if(ib==1) {a[n]=1.0; a[1]=1.0;}
	else {a[0]=1.0; a[1+n]=1.0;}
	ic[0]=1; ic[1]=1;
	return;
}
...
{
		u[0]=-sin(PI*x)*sin(PI*t);
		u[1]=cos(PI*x)*cos(PI*t);
		return;
}
