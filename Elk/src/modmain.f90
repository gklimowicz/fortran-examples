...
real(8) rndatposc
! tdatpos is .true. if small amplitude displacements are to be included when
! calculating the Coulomb potential
logical :: tdatpos=.false.
! trddatpos is .true. if the small amplitude displacements and velocities are to
! be read from file
logical :: trddatpos=.false.
! small amplitude atomic displacements and velocities
real(8) datposc(3,0:1,maxatoms,maxspecies)
...
integer maxitoep
! OEP step size and fraction of previous potential to be used as starting point
real(8) tauoep(2)
...
complex(8), parameter :: zil(0:3)=[zone,zi,-zone,-zi]
complex(8), parameter :: zilc(0:3)=[zone,-zi,-zone,zi]
! Pauli spin matrices:
...
!---------------------------------!
! code version
integer, parameter :: version(3)=[8,6,7]
