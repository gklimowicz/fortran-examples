...
module xc_f90_lib_m
  ...
  public :: &
    ...
    xc_f90_num_aux_funcs, &
    xc_f90_aux_func_ids, &
    xc_f90_aux_func_weights, &
    ! lda
    xc_f90_lda, &
    xc_f90_lda_exc, &
    xc_f90_lda_exc_vxc, &
 ...
 integer(c_int), parameter, public :: XC_GGA_X_Q1D = 734
  ...
  integer(c_int) function xc_f90_num_aux_funcs(p) result(naux)
    type(xc_f90_func_t), intent(in) :: p

    naux = xc_num_aux_funcs(p%ptr)
  end function xc_f90_num_aux_funcs

  subroutine xc_f90_aux_func_ids(p, ids)
    type(xc_f90_func_t), intent(in) :: p
    integer(c_int), intent(out) :: ids(:)

    call xc_aux_func_ids(p%ptr, ids)
  end subroutine xc_f90_aux_func_ids

  subroutine xc_f90_aux_func_weights(p, weights)
    type(xc_f90_func_t), intent(in) :: p
    real(c_double), intent(out) :: weights(:)

    call xc_aux_func_weights(p%ptr, weights)
  end subroutine xc_f90_aux_func_weights
