...
call init0
call init2
allocate(wq(nbph),w(nwplot),gw(nwplot),f(nwplot))
...
write(50,*)
write(50,'("(All quantities are per unit cell)")')
! zero point energy
do iw=1,nwplot
  f(iw)=gw(iw)*w(iw)
end do
...
deallocate(wq,w,gw,f,dynq,dynr,dynp,ev)
end subroutine
