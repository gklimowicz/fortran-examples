...
do ik=1,nkpt
  w=wkpt(ik)
  do ist=1,nstsv
    evalsum=evalsum+w*occsv(ist,ik)*evalsv(ist,ik)
  end do
end do
!------------------------!
!     kinetic energy     !
...
! kinetic energy from valence states
  ...
  do ik=1,nkpt
    ...
    do ist=1,nstsv
      ...
      engykn=engykn+w*occsv(ist,ik)*dble(z1)
    end do
  end do
...
! remove magnetic field contribution
  ...
  do idm=1,ndmag
    ...
    call rfmtctof(rfmt)
    sm=sm+rfinp(magmt(:,:,idm),magir(:,idm),rfmt,bsir(:,idm))
  end do
! remove fixed tensor moment potential matrix contribution
  ...
  engykn=evalsum-engyvcl-engyvxc-sm
  deallocate(rfmt)
end if
!-------------------------------!
  ...
  do ik=1,nkpt
    ...
    do ist=1,nstsv
      ...
      if ((f.gt.0.d0).and.(f.lt.1.d0)) then
        sm=sm+w*(f*log(f)+(1.d0-f)*log(1.d0-f))
      end if
    end do
  end do
! entropy
  entrpy=-occmax*kboltz*sm
