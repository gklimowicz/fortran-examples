/*	To find eigenvalues and eigenvectors of general real matrix 
...
main()
{
	...
	double hh, d[100],e[100], c[7][7],b,ei[20],eps,reps,p0,v[20],ev[7][7];
	double pi,pr;
...
/*	Perturb the eigenvalue slightly to avoid zero pivot 
	...
	can be correctly calculated, */
		...
		ier=invit(&c[0][0],n,id,&p0,v,iflg,&pi,&pr,reps,&np);
		for(j=0; j<n; ++j) {
			ev[j][i]=v[j];
		}
	} 
...
/*	Back transform the eigenvectors of balanced matrix */
		i=balbak(n,low,igh,&ev[0][0],n,id,d);
