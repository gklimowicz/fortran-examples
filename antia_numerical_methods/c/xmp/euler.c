...
main()
{
	int i,i1,j,n1,n2, m1,m2, iflg, ier,np;
	double xl, xu, c, rint, reps, aeps, df;

/*	Exercise 6.37 */

	aeps=1.e-19; reps=1.e-15;
	c=1.0;
	for(i1=0; i1<99; ++i1) {
		printf("type np = no. of terms,  m1, m2    (quits when np<0) \n");
		printf("     m1,m2 = No. of terms to be summed directly at two ends\n");
		scanf(" %d %d %d", &np, &m1, &m2);
		if(np<0) return 0;

		i=euler(np,&m1,&m2,c,reps,aeps,&df,&n1,&n2,&rint,fun);
		printf(" ier = %d  np = %d   m1 = %d    m2 = %d   n1 = %d  n2 = %d\n", i,np,m1,m2,n1,n2);
		printf(" sum = %e   estimated error = %e  \n", rint, df);
	}
	return;
}

/*  to calculate the terms without sign */
