/*	To solve parabolic equation in 2 space dimensions using alternating direction method */

#include <stdio.h>
#include <math.h>

void cof(double x, double y, double t, double *axx, double *ayy, double *ax,
		double *ay, double *au, double *a0);
double  bc(int ib, double x, double y, double t);
double fic(double x, double y, double t);
...
main()
{
	...
	double hh, x[100], wt[100],f[100][50],fc[100],tn,erc,reps,t0,fi;
	double x0,xn,dt,y0,yn,y[100];

/*	Example 14.14  */

	id=50;
	printf("type nx,ny = no. of points along x,y;  t0=initial time\n");
	scanf(" %d %d %le",&nx,&ny,&t0);
	x0=0.0; xn=1.0; iflg=0;
	y0=0.0; yn=1.0;

	for(i1=0; i1<99; ++i1) {
		printf("type nt = no. of time steps,    dt = step length\n");
		printf("                     (quits when nt<=0)\n");
		scanf(" %d %le",&nt,&dt);
		if(nt<=0) return 0;

		i=adm(&t0,dt,x0,xn,y0,yn,nt,nx,ny,x,y,&f[0][0],id,cof,bc,fic,&iflg);
		printf(" ier = %d  nx =  %d  ny = %d  nt = %d  t = %e  step length = %e \n",i,nx,ny,nt,t0,dt);
		printf("Solution : \n");

/*	Solution is printed row wise with successive rows containing solution
	for different values of y. For comparison exact solution is printed in
	the next line */

		for(i=0; i<ny; i += 5) {
			for(j=0; j<nx; j += 5) printf(" %e ",f[i][j]);
			printf(" \n");
			for(j=0; j<nx; j += 5) printf(" %e ",fic(x[j],y[i],t0));
			printf(" \n");
		}
...
/*	the coefficients in parabolic equation */

void cof(double x, double y, double t, double *axx, double *ayy,
		double *ax, double *ay, double *au, double *a0)
...
/*	The exact solution for generating initial values */
...
{
		return sin(PI*x)*sin(PI*y)*exp(-2.0*PI*PI*t);
}
