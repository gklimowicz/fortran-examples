/*	Recursive integration in 3 dimensions 
...
main()
{
	...
	double xl, xu, rint, reps, aeps, dif, a;
...
/*	ALP is the coordinate A in the exercise */
	for(i1=0; i1<99; ++i1) {
		printf("type ALP      (quits when a<-20) \n");
		scanf(" %le",&ALP);
		if(ALP<-20) return 0;
...
/*	The number of function evaluations are accumulated in global variable NP */
		...
		printf("ier = %d  ALP = %e    No. of function evaluations = %d \n",i,ALP,NP);
...
double fun(double x)
/*  the integrand is the integral over y and z */
...
/*	Store the value of x in the global variable XX for use by F1 and F2 */
	XX=x;
...
/*	Store the value of y in the global variable XX for use by F1 and F2 */
	YY=y;

	i=adpint(&ri,a,b,reps,aeps,&dif,f2,&n,&nmax);
	if(i>0) exit(1);
...
double f2(double z)
/* the integrand, the value of x, y are taken from global variable XX, YY */

{
	double af,f;
	af=(XX-ALP)*(XX-ALP)+YY*YY+z*z;
	f=0.0;
	if(af>0.0) f=4./sqrt(af);
	return f;
}
