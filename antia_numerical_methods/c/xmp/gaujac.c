/*	to calculate weights and abscissas of Gauss-Jacobi quadrature formulas 
...
main()
{
	...
	double alp, beta,  ri0, a[50], w[50];


	for(i1=0; i1<99; ++i1) {
		printf("type  nuse = No. of points in quadrature formula,  alp, beta \n");
		printf("                        (quits when n<=0)\n");
		scanf(" %d %le %le", &nuse,&alp,&beta);
		if(nuse<=0) return 0;

		i=gaujac(nuse,alp,beta,w,a);
		printf(" ier = %d  n = %d    alp = %e   beta = %e \n", i,nuse,alp,beta);
