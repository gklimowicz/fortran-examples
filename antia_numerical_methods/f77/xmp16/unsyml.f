!     PROGRAM FOR FINDING EIGENVALUES AND LEFT-EIGENVECTORS OF UNSYMMETRIC MATRIX
     ...
     1          ,INT(30),CWK(300)
!      DATA ((A(I,J),J=1,7),I=1,7)/6,0,0,0,0,1,0, 0,4,0,3Q-8,1Q-4,2Q-4,
!     1   1Q-3, 1,10000,7,0,0,-2,20, 0,2Q8,0,1,-40000,30000,-4Q5,
!     2   -2,-30000,0,1Q-4,2,2,40, 0,0,0,0,0,0,0,
!     3   0,1000,0,4Q-5,0.1Q0,-0.2Q0,3/
...
!     FINDING THE LEFT-EIGENVECTORS USING INVERSE ITERATION
...
!        CP=CMPLX(E(I),EI(I))*(1.+20.*REPS)+REPS**2
        CP=(E(I)+CI*EI(I))*(1.+20.*REPS)+REPS**2
        IFLG=0
        DO 2500 J=1,N
2500    CV(J,I)=1.0
        CALL INVIT_CL(AB,N,IA,CP,CV(1,I),IFLG,CEI,CR,REPS,CWK,INT,
     1               NIT,IER)
!        WRITE(6,58) IER,(CV(J,I),J=1,N)
3000  CONTINUE

!     BACK-TRANSFORM THE LEFT-EIGENVECTORS

      M=N
      IZ=30
      CALL BALBAK_L(N,LOW,IGH,CV,M,IZ,D)
...
!		initial approximation to left eigenvector. After execution it
...
!
      SUBROUTINE INVIT_CL(A,M,IA,P,U,IFLG,EI,ERC,REPS,WK,IWK,NIT,IER)
      IMPLICIT REAL*16(A,O,R)
      IMPLICIT COMPLEX*32(B-H,P,S-Z)
