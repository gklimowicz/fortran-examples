...
2300    FD=FD*X+A(K-J)
      ...
      FM=(FM-FUND(X)*FN/FD)*SIGN(1.Q0,SI)
      END
...
!
     ...
     1                 WK,IWK)
      IMPLICIT REAL*16(A-H,O-Z)
      EXTERNAL FM
      PARAMETER(NIT=30,NJT=10,NMAX=50,PI=3.14159265358979324Q0)
...
!	less than 1%
          IF(EMAX-EMIN.LT.1.Q-2*EMAX) RETURN
...
3600      CONTINUE
          ...
          EI=A(NK)
!	convergence check
          IF(DIF.LT.EPS.OR.DIF.LT.0.3Q0*(EMAX-EMIN)) GO TO 5000
          IF(IT.EQ.1.AND.IFLG.GE.1) EMAX=0.3Q0*ABS(A(NK))
4000    CONTINUE
