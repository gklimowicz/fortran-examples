...
!     FUNCTION WITH RANDOM ERROR TO GENERATE INPUT DATA
      ...
      F(Y)=EXP(-Y)+2.*EXP(-2.*Y)+3.0*EXP(-3.*Y)+RANGAU(SEED)*1.Q-6
...
!	If Hessian matrix appears to be singular, set the error flag
          IF(ABS(H2/H1).GT.1.3Q0) IER=53
          RETURN
        ENDIF

!	Initial guess for line search
        X1=0
        X2=MIN(1.Q0,-2.*DF/DF1)
        F1=F
        IF(X2.LE.0.0) X2=1
...
!
      ...
      IMPLICIT REAL*16(A-H,O,P,R-Z)
      EXTERNAL F
      PARAMETER(NIT=15,RHO=.01Q0,SIGMA=.1Q0,T1=9.,T2=.9Q0,T3=.5Q0)
...
!
      ...
      IMPLICIT REAL*16(A-H,O-Z)
      EXTERNAL F
      PARAMETER(NIT=25,T1=9.Q0,GR=1.618034Q0,GC=0.381966Q0)
