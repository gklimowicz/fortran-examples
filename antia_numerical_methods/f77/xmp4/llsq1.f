...
!     SINGULAR VALUE DECOMPOSITION IN ONE DIMENSION
 
      PROGRAM POLFIT
!      IMPLICIT REAL*8(A-H,O-Z)
      EXTERNAL PHI
      DIMENSION G(1000,50),X(9000),B(1000),FX(1000),V(50,50)
      DIMENSION YF(1000),WK(100),EF(1000),D(50),COV(50,50)
 
!     EXAMPLE 10.3
 
!     FUNCTION WITH RANDOM ERROR TO GENERATE INPUT DATA
 
      F(Y)=((231*Y*Y-315)*Y*Y+105)*Y*Y-5+1.D-3*RANGAU(SEED)
 
51    FORMAT('   IER =',I4,5X,'NO. OF DATA PTS =',I4,5X,
     1       'NO. OF BASIS FUNCTIONS =',I3/'   CHI SQUARE =',1PE14.6)
...
!     GENERATING THE DATA POINTS WITH RANDOM ERROR
      ...
      H=1.D0/(N-1.)
!	X is treated as one dimensional array
      DO 1000 I=1,N
        X(I)=(I-1)*H
1000  CONTINUE
      DO 2000 I=1,N
...
!      To calculate the set of basis functions in 1 dimension
!      These are X**(I-1)
