...
51    FORMAT('   X =',1PE14.6,'  A =',E14.6,'   B =',E14.6,/,
     1       '  Pgamma(A,X) =',E14.6,'    I_X(A,B) =',E14.6)
...
51    FORMAT(' ** Error in evaluating Incomplete Beta Function at',/,
     1  ' A=',1PE12.4,' B=',E12.4,' X=',E12.4,'  BETAP=',E12.4)
      ...
      IF(AMAX.LE.30) THEN
        ...
        ELSE
          ...
          BETAP=1-BETAP
        ENDIF
      ELSE IF(B.LE.20.AND.X.LE.0.71) THEN
        BETAP=BETSER(A,B,X)
      ELSE IF(A.LE.20.AND.X.GE.0.3) THEN
        BETAP=BETSER(B,A,1-X)
        BETAP=1-BETAP
      ELSE IF(B.LE.50.AND.X.LE.0.35) THEN
        BETAP=BETSER(A,B,X)
      ELSE IF(A.LE.50.AND.X.GE.0.65) THEN
        BETAP=BETSER(B,A,1-X)
        BETAP=1-BETAP
      ELSE IF(B.LE.100.AND.X.LE.0.18) THEN
        BETAP=BETSER(A,B,X)
      ELSE IF(A.LE.100.AND.X.GE.0.82) THEN
        BETAP=BETSER(B,A,1-X)
        BETAP=1-BETAP
      ELSE IF(B.LE.180.AND.X.LE.0.1) THEN
        BETAP=BETSER(A,B,X)
      ELSE IF(A.LE.180.AND.X.GE.0.9) THEN
        BETAP=BETSER(B,A,1-X)
        BETAP=1-BETAP
...
51    FORMAT(' ** Roundoff error while evaluating the infinite series',
     1  ' for incomplete Beta function at',/,'  A=',1PE12.4,'  B=',
     2  E12.4,'  X=',E12.4,'  SUM=',E12.4,'  MAX TERM=',E12.4)
      ...
      DO I=1,500,2
        ...
        IF(ABS(T/(A+I+1)).LT.1.E-9) EXIT
      ENDDO
      IF((S3).LT.1.E-9*TMAX) PRINT 51,A,B,X,S3,TMAX
...
!	Use continued fraction
        ...
        DO I=2,498,2
          ...
          IF(ABS(B(I+2)).GT.1.D30) THEN
            C(I+1)=C(I+1)/1.D30
            B(I+1)=B(I+1)/1.D30
            C(I+2)=C(I+2)/1.D30
            B(I+2)=B(I+2)/1.D30
          ENDIF
          IF(ABS(B(I+2)).LT.1.D-30) THEN
            C(I+1)=C(I+1)*1.D30
            B(I+1)=B(I+1)*1.D30
            C(I+2)=C(I+2)*1.D30
            B(I+2)=B(I+2)*1.D30
