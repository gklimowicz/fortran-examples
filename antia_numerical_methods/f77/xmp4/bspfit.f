!     LEAST SQUARES FIT USING B-SPLINES IN 1 DIMENSION

      PROGRAM BSP
!      IMPLICIT REAL*8(A-H,O-Z)
      EXTERNAL RAN1
      DIMENSION XO(90),T(100),B(200),AX(200,90),WK(900),F(90),XF(90)
      DIMENSION EF(2000),Y0(2000),AY(200,50),COV(90,90)
 
!     EXAMPLE 10.3

!	Single precision version of this example uses a different
!	random errors and hence the results are much better as compared
!	to double precision version.

!      FUN(Y)=((231*Y*Y-315)*Y*Y+105)*Y*Y-5+1.D-5/(RAN1(S)-0.5)**3
      FUN(Y)=((231*Y*Y-315)*Y*Y+105)*Y*Y-5+1.D-3*(RAN1(S)-0.5)
...
51    FORMAT('   IER =',I4,3X,'NO. OF POINTS =',I5,3X,'NO. OF KNOTS =',
     ...
     4       'COEFFICIENTS :'/(5E14.6))
52    FORMAT('   IER =',I4,3X,'X =',1PE14.6,5X,'F(X) =',E14.6/
     1       5X,7HF'(X) =,E14.6,5X,8HF''(X) =,E14.6)
...
!     set the seed here so that the same random numbers are used every time
      S=2

!     Setup the knots and table of values using known function with
!     random error added
      H=1.D0/(NX-1.)
      DO 1000 I=1,NX
        XO(I)=(I-1)*H
