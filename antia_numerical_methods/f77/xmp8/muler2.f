...
!	If we try to start from larger zeros, then Muller fails to
!	find them as iteration always tends to go towards small X.
!	But if we start with small starting values then all zeros
!	can be determined without much difficulty as the smaller
!	zeros are then deflated.
