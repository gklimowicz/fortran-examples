!============================================================================
!                CTEQ-TEA Parton Distribution Functions: version 2018
!                       December 31, 2018
!
...
!   (unpublished), CT14 and CT18 parton distribution functions. 
!
...
!   (9) 4 CT14 NNLO sets with intrinsic charm     [4]
!   (10) 1+56 sets of CT14HERA2 NNLO PDF's;       [5]
!   (11) 1+56 sets of CT14HERA2 NLO PDF's;        [5]
!   (12) 1+58 sets of CT18 NNLO PDF's;            [6]
!   (13) 1+58 sets of CT18 NLO PDF's;             [6]
!   (14) 2 sets of CT18 LO PDF's;                 [6]
!   (15) 10 CT18 NNLO sets and 10 CT18 NLO sets   [6]
!       with alternative alpha_s values         
!   (16) 3 CT18 NNLO and 3 CT18 NLO sets          [6]
!       with up to 3, 4, and 6 active quark flavors
!
!   References
!   [1] J. Gao, M. Guzzi, J. Huston, H.-L. Lai, Z. Li, P. M. Nadolsky, 
!       J. Pumplin, D. Stump, C.-P. Yuan,  arXiv:1302.6246 [hep-ph]
!       https://hep.pa.msu.edu/cteq/public/ct10.html
...
!       C.-P. Yuan, arXiv:1506.07443
!   [4] T.-J. Hou, S. Dulat, J. Gao, M. Guzzi, J. Huston,
!       P. M. Nadolsky, C. Schmidt, J. Winter, K. Xie, and
!       C.-P. Yuan, arXiv:1707.00657
!   [5] T.-J. Hou, S. Dulat, J. Gao, M. Guzzi, J. Huston,
!       P. M. Nadolsky, J. Pumplin, C. Schmidt, D. Stump and
!       C.-P. Yuan, arXiv:1609.07968
!   [6] T.-J. Hou, K. Xie, J. Gao, S. Dulat, M. Guzzi,
!       T. J. Hobbs, J. Huston, P. M. Nadolsky, J. Pumplin,
!       C. Schmidt, I. Sitiwaldi, D. Stump and C.-P. Yuan, 
!       arXiv:0000.00000
! 
!   The correspnding PDFs can be downloaded from the following websites:
!   CT10:  
!   https://hep.pa.msu.edu/cteq/public/ct10.html
!   CT14/CT14HERA2:  
!   https://hep.pa.msu.edu/cteq/public/ct14.html
!   CT18:  
!   https://hep.pa.msu.edu/cteq/public/ct18.html
!
! ===========================================================================
...
!   The function CT18Pdf (Iparton, X, Q)
...
!     Call CT18GetPars( xmin,Qini,Qmax,Nloops,Nfl), 
!   which returns
...
!   precision.
!   If you have detailed questions concerning these CT18 distributions,
...
contains
  ...
  Function getCT18Pdf (Iparton, X, Q)
    ...
    if (ipdsset.ne.1) STOP 'getCT18Pdf: the PDF table was not initialized'
    
    If (X .lt. 0d0 .or. X .gt. 1D0) Then
       Print *, 'X out of range in getCT18Pdf: ', X
       getCT18Pdf = 0D0
       Return
    Endif
    
    If (Q .lt. Qsml) Then
       Print *, 'Q out of range in getCT18Pdf: ', Q
       Stop
    Endif
...
!        print a warning for calling extra flavor
          Warn = .false.
          Print *, 'Warning: Iparton out of range in getCT18Pdf! '
          Print *, 'Iparton, MxFlvN0: ', Iparton, NfMx
       Endif
       getCT18Pdf = 0D0
    else
       
       getCT18Pdf = PartonX12 (Iparton, X, Q)
       if (getCT18Pdf.lt.0D0) getCT18Pdf = 0D0
    endif                     !if (abs(Iparton...
    
    Return

!                             ********************
  End Function getCT18Pdf

  Subroutine SetCT18(prefix,Iset)    
    Implicit Double Precision (A-H,O-Z)
    ...
    If(Iset .eq. 1) Then
       Tablefile='CT18n.00.pds'
    Else if (Iset .eq. 2) Then
       Tablefile='CT18nn.00.pds'
    Else
       Print *, 'Invalid Iset number in SetCT18 :', Iset
       Stop
    End if
...
100 Print *, ' Data file ', prefix // "/" // trim(Tablefile), &
         ' cannot be opened in SetCT18!!'
    Stop
!                             ********************
  End Subroutine SetCT18
...
!         Nfl   is the maximal number of quark flavors
  ...
  Function CT18Alphas (QQ)
    ...
    if (ipdsset.ne.1) STOP 'CT18Alphas: the PDF table was not initialized'
    
      
    if (ipdsformat.lt.11) then
       print *
       print *, 'STOP in CT18alphas: the PDF table file has an older format'
       print *, 'and does not include the table of QCD coupling values.'
       print *, 'You can still compute the PDFs, but do not call'
       print *, 'the CT18alphas function for the interpolation of alpha_s.'
       stop
    endif
...
!     u  d  s  c  b  t
    ...
    if (ipdsset.ne.1) STOP 'CT18Mass: the PDF table was not initialized'
    
    CT18Mass = Amass(i)
    
    return 
  end function CT18Mass
