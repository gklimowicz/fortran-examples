...
if test -f OCAML_FLAG; then
    ...
    echo "Contents of ${name}_raw.weights.dat" >> $name.log
    cat ${name}_raw.weights.dat >> $name.log
    echo "Contents of ${name}_lhef.weights.dat" >> $name.log
    cat ${name}_lhef.weights.dat >> $name.log
