...
program check_r2c_3d
  ...
  integer(C_INT),parameter  :: N3r=N3/2+1  
  
  real(C_DOUBLE), dimension(N1,N2,N3) :: Input
  complex(C_DOUBLE_COMPLEX), dimension(N1,N2,N3r) :: Out,Output,Output_ref
  real(C_DOUBLE), allocatable, dimension(:) :: Rhor
  complex(C_DOUBLE_COMPLEX), allocatable, dimension(:) :: Rhok
...
! Initialize OpenFFT  
  ...
  t3 = openfft_init_r2c_3d(%VAL(N1),%VAL(N2),%VAL(N3),&
...
! FFT transform 
  ...
  t3 = openfft_exec_r2c_3d(Rhor, Rhok)
...
! Gather results from all processes 
  ...
  call MPI_Allreduce(Out,Output,N1*N2*N3r,MPI_DOUBLE_COMPLEX,MPI_SUM, &
...
! Print global output                                                           
  if (nrank==0) then
     ...
     do i=1,N1
        ...
        write(*,*) ''
        do j=1,N2
           write(*,20) (real(Output(i,j,k)),k=1,N3r)
           write(*,21) (aimag(Output(i,j,k)),k=1,N3r)
           write(*,*) ''
        end do
     end do
  end if
...
! Error check 
  ...
  Output_ref(1,1,1) = ( 3.292, 0.000)
  Output_ref(1,1,2) = ( 0.051,-0.144)
  Output_ref(1,1,3) = ( 0.113, 0.000)
  Output_ref(1,2,1) = ( 0.143,-0.188)
  Output_ref(1,2,2) = ( 0.016, 0.051)
  Output_ref(1,2,3) = (-0.024, 0.025)
  Output_ref(1,3,1) = ( 0.143, 0.188)
  Output_ref(1,3,2) = (-0.050, 0.016)
  Output_ref(1,3,3) = (-0.024,-0.025)
  Output_ref(2,1,1) = ( 1.225, 0.000)
  Output_ref(2,1,2) = ( 0.000, 0.000)
  Output_ref(2,1,3) = ( 0.000, 0.000)
  Output_ref(2,2,1) = ( 0.000, 0.000)
  Output_ref(2,2,2) = ( 0.000, 0.000)
  Output_ref(2,2,3) = ( 0.000, 0.000)
  Output_ref(2,3,1) = ( 0.000,-0.000)
  Output_ref(2,3,2) = (-0.000, 0.000)
  Output_ref(2,3,3) = ( 0.000,-0.000)
