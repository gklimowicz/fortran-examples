...
{
  int i,j,BN_AB,BN_CB,BN_CA,gp,NN_S,NN_R,NN_RS,BN_ABf;
  ...
  */
  ...
  for (BN_AB=0; BN_AB<My_NumGrid_AB; BN_AB+=OFFT_Ngrid3){
    BN_ABf += OFFT_Ngrid3f;
    /*
    for (i=0; i<OFFT_Ngrid3f; i++){
      OFFT_rin[i] = Rhor[BN_ABf+i];
    }
    */
    memcpy(&OFFT_rin[0], &Rhor[BN_ABf], OFFT_Ngrid3f*SIZE_DOUBLE);

    if (OFFT_measure_time==1) openfft_dtime(&time00);

    fftw_execute(OFFT_p_C);
    ...
    /*
    for (i=0; i<OFFT_Ngrid3; i++){
      Rhot[BN_AB+i].r = OFFT_out[i][0];
      Rhot[BN_AB+i].i = OFFT_out[i][1];
    }
    */
    memcpy(&Rhot[BN_AB],&OFFT_out[0][0],OFFT_Ngrid3*SIZE_DCOMPLEX);
  }

  if (OFFT_measure_time==1){
    openfft_dtime(&time1);
    timeDf += time1 - time0 - timeT;
  }
...
case 1: {
  ...
  for (ID=0; ID<NN_AB2CA_S; ID++){
	...
	array0[gp+i].r = Rhot[BN_AB].r;
	array0[gp+i].i = Rhot[BN_AB].i;
	/*
	memcpy(&array0[gp+i],&Rhot[BN_AB],SIZE_DCOMPLEX);
	*/
      } 
    }
  }
  ...
  for (ID=0; ID<NN_AB2CA_R; ID++){
	...
	Rhok[BN_CA].r = array1[gp+i].r;
	Rhok[BN_CA].i = array1[gp+i].i;
	/*
	memcpy(&Rhok[BN_CA],&array1[gp+i],SIZE_DCOMPLEX);
	*/
      }
    }
    else{
	...
	BN_CA = Index_Rcv_Grid_AB2CA[IDR][i];
	Rhok[BN_CA].r = Rhot[BN_AB].r;
	Rhok[BN_CA].i = Rhot[BN_AB].i;
	/*
	memcpy(&Rhok[BN_CA],&Rhot[BN_AB],SIZE_DCOMPLEX);
	*/
      }
    }
  }
...
break;
 ...
 for (BN_CA=0; BN_CA<My_NumGrid_CA; BN_CA+=OFFT_Ngrid2){
    /*
    for (i=0; i<OFFT_Ngrid2; i++){
      OFFT_in[i][0] = Rhok[BN_CA+i].r;
      OFFT_in[i][1] = Rhok[BN_CA+i].i;
    }
    */
    memcpy(&OFFT_in[0][0], &Rhok[BN_CA], OFFT_Ngrid2*SIZE_DCOMPLEX);

    if (OFFT_measure_time==1) openfft_dtime(&time00);

    fftw_execute(OFFT_p_B);
    ...
    /*
    for (i=0; i<OFFT_Ngrid2; i++){
      Rhok[BN_CA+i].r = OFFT_out[i][0];
      Rhok[BN_CA+i].i = OFFT_out[i][1];
    }
    */
    memcpy(&Rhok[BN_CA], &OFFT_out[0][0], OFFT_Ngrid2*SIZE_DCOMPLEX);
  }
...
case 1:{
  ...
  for (ID=0; ID<NN_CA2CB_R; ID++){
	...
	Rhot[BN_CB].r = array1[gp+i].r;
	Rhot[BN_CB].i = array1[gp+i].i;
	/*
	memcpy(&Rhot[BN_CB],&array1[gp+i],SIZE_DCOMPLEX);
	*/
      }
    }
    else{
	...
	BN_CB = Index_Rcv_Grid_CA2CB[IDR][i];
	Rhot[BN_CB].r = Rhok[BN_CA].r;
	Rhot[BN_CB].i = Rhok[BN_CA].i;
	/*
	memcpy(&Rhot[BN_CB],&Rhok[BN_CA],SIZE_DCOMPLEX);
	*/
      }
    }
  }
...
break;
  ...
  for (BN_CB=0; BN_CB<My_NumGrid_CB; BN_CB+=OFFT_Ngrid1){
    /*
    for (i=0; i<OFFT_Ngrid1; i++){
      OFFT_in[i][0] = Rhot[BN_CB+i].r;
      OFFT_in[i][1] = Rhot[BN_CB+i].i;
    }
    */
    memcpy(&OFFT_in[0][0], &Rhot[BN_CB], OFFT_Ngrid1*SIZE_DCOMPLEX);

    if (OFFT_measure_time==1) openfft_dtime(&time00);

    fftw_execute(OFFT_p_A);
    ...
    /*
    for (i=0; i<OFFT_Ngrid1; i++){
      Rhok[BN_CB+i].r = OFFT_out[i][0];
      Rhok[BN_CB+i].i = OFFT_out[i][1];
    }
    */
    memcpy(&Rhok[BN_CB], &OFFT_out[0][0], OFFT_Ngrid1*SIZE_DCOMPLEX);
  }
