...
if (mp_mpi) then
  ...
  write(*,'("Total number of threads (MPI x OpenMP) : ",I6)') np_mpi*maxthd
  write(*,'("Maximum OpenMP nesting level : ",I4)') maxlvl
  write(*,'("Number of OpenMP threads at first nesting level : ",I4)') maxthd1
  write(*,'("Number of MKL threads : ",I4)') maxthdmkl
...
! initialise the RAM disk if required
if (ramdisk) then
  call initrd
  if (mp_mpi) then
    write(*,*)
    write(*,'("RAM disk enabled")')
    if (.not.wrtdsk) then
      write(*,*)
      write(*,'("Warning(elk): Some direct access files may not be written to &
       &disk")')
    end if
  end if
else
...
! increment the batch variables if required
  if (batch) call batchdv
! synchronise MPI processes
  call mpi_barrier(mpicom,ierror)
! check if task can be run with MPI
  if (lp_mpi.gt.0) then
    if (any(task.eq.[0,1,2,3,5,15,16,28,29,61,62,63,110,120,121,125,135,136, &
     162,170,180,185,188,200,201,205,208,240,241,270,271,300,320,330,331,350, &
     351,371,372,373,440,460,461,462,463,471,478,600,610,620,630,640,700,701]))&
     then
      continue
    else
...
!BOI
! !TITLE: {\huge{\sc The Elk Code Manual}}\\ \Large{\sc Version 8.6.7}\\ \vskip 20pt \includegraphics[height=1cm]{elk_silhouette.pdf}
...
!   Bartolom\'{e}, Ren\'{e} Wirnata, Jagdish Kumar, Andrew Shyichuk, Nisha
!   Singh, Pietro Bonfa, Ronald Cohen, Alyn James, Chung-Yu Wang, Leon Kerber,
!   Yunfan Liang, Xavier Gonze and Mike Bruckhoff. Special mention of David
...
!   acknowledge the generous support of Karl-Franzens-Universit\"{a}t Graz,
!   the EU Marie-Curie Research Training Networks initiative, the Max Born
...
!
!   \vspace{12pt}
!   Berlin, Halle, Jerusalem and Uppsala, December 2022
!   \newpage
!
!   \section{Units}
...
!   0.529177210903(80) \AA, and the atomic unit of energy is the Hartree which
!   equals 27.211386245988(53) eV. The unit of the external magnetic fields is
!   defined such that one unit of magnetic field in {\tt elk.in} equals
!   1715.255541 Tesla.
!
!   \section{Compiling and running Elk}
!   \subsection{Compiling the code}
!   Unpack the code from the archive file. Edit the file {\tt make.inc} in the
!   {\tt elk} directory and adjust the compiler options for your computer
!   system. Use of machine-optimised BLAS/LAPACK and FFT libraries will result
!   in significant increase in performance. Following this, run
!   \begin{verbatim}
!     make
!   \end{verbatim}
...
!    \end{verbatim}
!    where n is the number of cores available on a particular node. The same can
!    be accomplished in {\tt elk.in} with
!    \begin{verbatim}
!     maxthd
!      n
!    \end{verbatim}
!    In addition, some vendor-supplied BLAS/LAPACK libraries use OpenMP
!    internally. The maximum number of threads used for LAPACK operations by
!    Intel's MKL can be set with
!    \begin{verbatim}
!     maxthdmkl
!      n
!    \end{verbatim}
...
!   Elk is a memory-bound code and runs best on processors with large caches and
!   a large number of memory channels per core. Some tasks in Elk require a
!   considerable amount of memory which can exceed the physical memory of the
!   computer. In such cases, the number of threads at the first nesting level
!   can be reduced with (for example)
!   \begin{verbatim}
!    maxthd1
!     -4
!   \end{verbatim}
!   which restricts the number of threads at the first nesting level to
!   maxthd/4. Deeper nesting levels, which generally require less memory, will
!   still utilise the full compliment of available threads.
!   \subsubsection{Stack space}
!   The latest versions of Elk use stack space aggressively. This is because
!   accessing variables is faster on the stack than on the heap. This can,
!   however, result in the code crashing as threads run out of their stack
!   space. To avoid this, increase the stack size for each OpenMP thread with
!   (for example)
!   \begin{verbatim}
...
!   compile Libxc version 5. This should have produced the files {\tt libxc.a}
!   and {\tt libxcf90.a}. Copy these files and the file {\tt libxcf90.f90} from
!   the Libxc {\tt src} directory to the {\tt elk/src} directory and then
!   uncomment the lines indicated for Libxc in the file {\tt elk/make.inc}. Once
!   this is done, run {\tt make clean} followed by {\tt make}. To select a
...
!   {\tt elk/src/libxcf90.f90} for a list of the functionals and their
!   associated numbers.
!
...
!   for example, set up an antiferromagnetic crystal by pointing the field on
...
!    optimisation run & real & $5\times 10^{-3}$}
...
!   moment is broken but no FTM calculation is performed. See also {\tt tm3fix}.
!
!   \block{fxclrc}{
...
!
!   \block{tauoep}{
!   {\tt tauoep} & step length and starting fraction for the OEP iterative
!    solver & real(2) & $(0.05, 0.95)$}
...
!   described in the article. The second number defines the fraction of the
!   existing potential to be used as the starting point of the iterative
!   procedure. These parameters are also used for inverting the Kohn-Sham
!   equations as required by the self-consistent density $GW$ method. See
!   {\tt maxitoep}.
!
!   \block{taufsm}{
...
!   {\tt l(i)} & $l$ of TM & integer & - \\
!   \hline
!   {\tt (k, p, r, t)(i)} & indices for the 3-index TM & integer & - \\
!   \hline
!   {\tt wkpr(t)(i)} & real TM value & real & - }
!   This block sets up the fixed tensor moment (FTM). There should be as many
!   TM entries as {\tt ntmfix}. See the routine {\tt tm3todm} for the tensor
!   moment indexing convention.
!
!   \block{tmwrite}{
...
!   being adhered to:
!   \begin{itemize}
!   \item Strict Fortran 2008 should be used. Features which are marked as
!    obsolescent in Fortran 2008 should be avoided. These include assigned
