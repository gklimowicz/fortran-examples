/*	Linear least square fit using SVD in 3 dimensions */
...
main()
{
	...
	double hh, x[100], f[20][20][20], ax[900000],ay[10000],
	reps, xx,yy, chi, a[20][20],y[200],fy[20][20][20],cov[10000];
	double xy[10000][5],x0[9],df[9],d2f[3][3],ef[20][20][20],c[20000];

	seed=11; np=100; id=5; reps=1.e-7;
/*	No. of data points is fixed by dimensions */
	nx=20;

/*	For fit in 3 dimensions the number of basis functions should be M0**3
	where M0 is the number along each dimension */

	printf("type m = No. of basis functions\n");
	scanf(" %d", &m);
	...
	for(i=0; i<nx; ++i) {
		for(j=0; j<nx; ++j) {
			for(i1=0; i1<nx; ++i1) {
				f[i1][j][i]=fun(x[i],x[j],x[i1]);
				ef[i1][j][i]=1.e-3;
				xy[i+nx*j+nx*nx*i1][0]=x[i];
				xy[i+nx*j+nx*nx*i1][1]=x[j];
				xy[i+nx*j+nx*nx*i1][2]=x[i1];
			}
		}
	}
	nt=nx*nx*nx;

	j=llsq(nt,m,nd,&xy[0][0],id,&f[0][0][0],&ef[0][0][0],c,ax,ay,np,np,y,&fy[0][0][0],phi,reps,&chi,cov);
	...
	for(i=0; i<m1; ++i) {
		for(i1=0; i1<m1; ++i1) {
			for(j=0; j<m2; ++j) {
				...
				if(i>0) f1=pow(x[0],(double ) i);
				if(i1>0) f2=pow(x[1],(double ) i1);
				if(j>0) f3=pow(x[2],(double ) j);
				f[i+j*m1*m1+i1*m1]=f1*f2*f3;
			}
		}
	}
	return;
