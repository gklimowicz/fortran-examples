/*	To integrate   f(x)*exp(-x*x)  over infinite interval */
...
main()
{
	...
	double xl, xu, rint, reps, aeps, df;

/*	Exercise 6.24  (I1) */

	aeps=1.e-19; reps=1.e-14;
	i=hermit(&rint,aeps,reps,&df,fun,&nuse);
	printf(" ier = %d  no. of function evaluations = %d \n", i,nuse);
	printf(" integral = %e   estimated error = %e  \n", rint, df);
	return;
}

/*	The integrand for hermit */
