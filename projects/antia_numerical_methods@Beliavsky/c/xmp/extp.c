...
/*	Example 12.4 */
	...
	id=4; n=1; nmax=100000; reps=1.e-8;
	printf("type t0=initial time,   hh=initial step size,   al=Lamdba \n");
	scanf(" %le %le %le",&t0,&hh,&al);
	printf("type  iflg=0/1  for polynomial/rational function extrapolation\n");
	scanf(" %d",&iflg);
	printf("type the initial values \n");
	for(i=0; i<n; ++i) scanf(" %le",&x[i]);
	printf("  lambda = %e   t0 = %e   initial step size = %e    iflg = %d \n",al,t0,hh,iflg);
	...
	for(i1=0; i1<99; ++i1) {
		...
		i=extp(n,x,dx,dif,&hh,&t0,tn,reps,&np,&nmax,iflg);
		m=m+np;
		printf(" ier = %d    no. of function evaluations =  %d    t = %e  step length = %e \n",i,m,tn,hh);
		printf("solution : ");
		for(i=0; i<n; ++i) printf(" %e ",x[i]);
		printf(" \n");
