!     PROGRAM TO SOLVE LINEAR SECOND ORDER ELLIPTIC EQUATIONS IN TWO DIMENSIONS
!     USING ALTERNATING DIRECTION IMPLICIT ITERATIVE (ADI) METHOD
...
!     K IS THE PARAMETER IN ADI METHOD, 2**K SHOULD BE APPROXIMATELY
!     EQUAL TO THE NUMBER OF ITERATIONS REQUIRED.
...
!	Required routines : COF, BC
     ...
     1               AEPS,NIT,WK,WKA)
      IMPLICIT REAL*16(A-H,O-Z)
      PARAMETER(PI2=1.570796326794896619Q0,MAXIT=1000,KM=6,NK=2**(KM-1))
...
!     THE COEFFICIENTS OF LINEAR SECOND ORDER ELLIPTIC EQUATION
...
!     BOUNDARY CONDITIONS  U(0,Y)=U(1,Y)=U(X,0)=0,  U(X,1)=SIN(PI*X)
