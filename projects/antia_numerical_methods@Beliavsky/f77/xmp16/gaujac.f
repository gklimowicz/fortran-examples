...
!!    THIS PROGRAM REQUIRES GAMMA FUNCTION, WHICH MAY NOT BE CALCULATED
!     TO FULL QUAD PRECISION ACCURACY. HENCE FOR ALL VALUES OF ALP AND
!     BETA THE RESULT MAY NOT HAVE REQUIRED ACCURACY. 

      PROGRAM GAUSSL
      IMPLICIT REAL*16(A-H,O-Z)
      DIMENSION X(65),W(65),COF(3,65),WK(9000),RIN(0:65)
 
51    FORMAT(5X,'IER =',I4,'    N =',I4,5X,'ALP =',1PD14.6,5X,'BETA =',
     1       D14.6/9X,'ABSCISSAS',10X,'WEIGHTS'/(D24.16,3X,D24.16))
52    FORMAT(/'   MAXIMUM ERROR USING ',I4,' POINTS =',1PD12.4)
...
!     Only N=0,1,2 are used, so the test is not complete
      ...
      ERR=0.0
      RIN(0)=2.Q0**(ALP+BETA+1)*GAMMA(ALP+1.0)*GAMMA(BETA+1.0)/
...
!	of ALP and BETA if the integral of weight function RI0 can
...
!     Required routines : GAUSRC, TQL2, GAMMA
      ...
      RI0=2.Q0**(ALP+BETA+1)*GAMMA(ALP+1.0)*GAMMA(BETA+1.0)/
     1       GAMMA(ALP+BETA+2.0)

!     Coefficients of recurrence relation
      WK(1,1)=1.Q0+(ALP+BETA)/2.0
      WK(2,1)=(ALP-BETA)/2.0
...
!	Hence do not calculate it.
      ...
      DO 2500 I=2,N+1
        ...
        WK(3,I)=2.0*(I-1.Q0+ALP)*(I-1.0+BETA)*(ALP+BETA+2*I)/A1
2500  CONTINUE
