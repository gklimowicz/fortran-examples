...
########################################################################
while read module threshold abs_threshold n roots model sign mode process; do
...
cat <<EOF
program compare_driver_majorana
  use kinds
  use compare_lib
EOF
...
cat <<EOF
  ...
  call check (load_v1_$module (), load_v2_$module (), &
              ...
              failures = failures, attempts = attempts, flip_sign = $sign)
  if (failures > 0) then
...
end program compare_driver_majorana
EOF
