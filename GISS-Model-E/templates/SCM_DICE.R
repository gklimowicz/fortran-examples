...
Atmosphere-only test from DICE model intercomparison.
...
SCM_PS=SCM_DICE_scalar.nc      ! surface pressure
SCM_USTAR=SCM_DICE_scalar.nc   ! surface friction velocity
SCM_SFLUX=SCM_DICE_scalar.nc   ! surface heat fluxes
SCM_TSKIN=SCM_DICE_scalar.nc   ! skin temperature
SCM_WIND=SCM_DICE_init.nc      ! initial horizontal wind profiles
SCM_GEO=SCM_DICE_force.nc      ! geostrophic wind profiles
SCM_THETA=SCM_DICE_init.nc     ! initial potential temperature profile
SCM_WVMR=SCM_DICE_init.nc      ! initial water vapor mixing ratio profile
SCM_OZONE=SCM_DICE_init.nc     ! ozone profile
SCM_OMEGA=SCM_DICE_force.nc    ! large-scale vertical wind profiles
SCM_LS_H=SCM_DICE_force.nc     ! large-scale horiz adv flux div profile(s)
SCM_LS_H_UV=SCM_DICE_force.nc  ! large-scale horiz adv wind div profile(s)
...
! SCM parameters
SCM_lon=-96.735            ! CASES-99 experiment longitude (deg)
SCM_lat=37.65              ! CASES-99 experiment latitude (deg)
...
&&END_PARAMETERS
 ...
 &INPUTZ
 YEARI=1999,MONTHI=10,DATEI=23,HOURI=19, ! pick IYEAR1=YEARI (default) or < YEARI
 YEARE=1999,MONTHE=10,DATEE=26,HOURE=19,     KDIAG=12*0,9,
 ISTART=2,IRANDI=0,
/
