...
!
      ...
      use hycom_arrays, only: scp2,scux,scvx,scpx,scuy,scvy,scpy &
          ,latij,lonij
! --- compute mesh size at u,v,p,q points
      implicit none
!
...
!
! --- read in array of lat/lon values
      ...
      close (unit=16)
      write(*,'(2a)') 'chk latlon =>',trim(path0)//latlonij
      print *,'lat/lon(243,180)=',lat4(243,180,3),lon4(243,180,3)
!
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
! --- optional: locate north pole in grid
      hilat=-99.
      do 1 j=jz,1,-1
      do 1 i=1,iz
...
! --- dist.(m) between 2 points on sphere, lat/lon (x1,y1) and lat/lon (x2,y2)
      implicit none
...
!     if (sphdis.eq.0.) write (*,'(a,2f8.3,2x,2f8.3)')			&
!       'warning - zero distance between lat/lon points',x1,y1,x2,y2
      sphdis=max(sphdis,1.)
      return
      end
