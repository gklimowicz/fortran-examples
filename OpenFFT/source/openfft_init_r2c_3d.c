			   ...
			   int measure_time, int print_memory)
{
  ...
  unsigned long long int BN_AB,BN_CB,BN_CA;
  unsigned long long int GNs,n2D,N2D;
  unsigned long long int GN_AB,GN_CB,GN_CA;
  int n1,n2,n3,blocksize,i,comm_patt_no,l;
  MPI_Status stat;
  MPI_Request request;
  double time0,TStime,TEtime;
  int myStart,myEnd;
  double MB,time,time1,time_min;
  int size_array0,size_array1,size_array;
  ...
  ****************************************************/
  ...
  /*
    added by hagita@nda.ac.jp
  */
  OFFT_rin  = fftw_malloc(sizeof(double)*OFFT_Ngrid3f); 

  OFFT_in  = fftw_malloc(sizeof(fftw_complex)*Max_OneD_Grids); 
  OFFT_out = fftw_malloc(sizeof(fftw_complex)*Max_OneD_Grids); 

  /*
    replaced by hagita@nda.ac.jp
  */
  OFFT_p_C = fftw_plan_dft_r2c_1d(OFFT_Ngrid3f,OFFT_rin,OFFT_out,
				  FFTW_ESTIMATE);
  /*
  OFFT_p_C = fftw_plan_dft_1d(OFFT_Ngrid3f,OFFT_in,OFFT_out,-1,
			      FFTW_ESTIMATE);
  */

  OFFT_p_B = fftw_plan_dft_1d(OFFT_Ngrid2,OFFT_in,OFFT_out,-1,FFTW_ESTIMATE);
  OFFT_p_A = fftw_plan_dft_1d(OFFT_Ngrid1,OFFT_in,OFFT_out,-1,FFTW_ESTIMATE);
  ...
  */
  ...
  /*
    replaced by hagita@nda.ac.jp
  */
  myStart = ((myid*N2D+numprocs-1)/numprocs)*OFFT_Ngrid3f;
  myEnd   = (((myid+1)*N2D+numprocs-1)/numprocs)*OFFT_Ngrid3f - 1;
  /*
  ...
  */
  ...
  /*
    replaced by hagita@nda.ac.jp
  */
  My_Index_In[1] = (myStart-My_Index_In[0]*N2D)/OFFT_Ngrid3f;
  My_Index_In[2] = myStart-My_Index_In[0]*N2D-My_Index_In[1]*OFFT_Ngrid3f;
  /*
  ...
  */
  ...
  /*
    replaced by hagita@nda.ac.jp
  */
  My_Index_In[4] = (myEnd-My_Index_In[3]*N2D)/OFFT_Ngrid3f;
  My_Index_In[5] = myEnd-My_Index_In[3]*N2D-My_Index_In[4]*OFFT_Ngrid3f;
  /*
  ...
  */
  ...
  if(My_Index_In[0]==My_Index_In[3]){
    OFFT_My_NumGrid_In = (My_Index_In[4]-My_Index_In[1]+1)*OFFT_Ngrid3f; 
   }
   else if(My_Index_In[0]<My_Index_In[3]){
     OFFT_My_NumGrid_In = OFFT_Ngrid2-My_Index_In[1];
     OFFT_My_NumGrid_In += (My_Index_In[3]-1-My_Index_In[0])*OFFT_Ngrid2;
     OFFT_My_NumGrid_In += My_Index_In[4]+1;
     OFFT_My_NumGrid_In *= OFFT_Ngrid3f;
   }
  ...
  */
  ...
  if (OFFT_My_Max_NumGrid<OFFT_My_NumGrid_In) OFFT_My_Max_NumGrid = OFFT_My_NumGrid_In;
  ...
  *My_NumGrid_Out = OFFT_My_NumGrid_Out;
  ...
  Rhot = (dcomplex*)malloc(sizeof(dcomplex)*OFFT_My_Max_NumGrid);
  ...
  if(OFFT_MEASURE == -1){
    ...
    Rhor = (double*)malloc(sizeof(double)*OFFT_My_Max_NumGrid); 
  ...
  if(!myid) 
    printf("\nExecuting with communication pattern number %d.\n\n",
	   COMM_PATT);
  ...
  if(OFFT_print_memory==1 && myid==0){
    ...
    printf("Temporary OFFT_rin   : %10.2f MB\n",
	   (double)(sizeof(double)*OFFT_Ngrid3f/MB));
